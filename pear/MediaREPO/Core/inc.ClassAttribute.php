 ...
 *
 * @see mediarepo_Core_AttributeDefinition
 *
 * @category   REPO
 * @package    mediarepo_Core
 * @author     Uwe Steinmann <uwe@steinmann.cx>
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_Attribute { /* {{{ */
	/**
	...
	/**
	 * @var mediarepo_Core_Folder|mediarepo_Core_Document|mediarepo_Core_DocumentContent mediarepo_Core_Object folder, document or document content
	...
	/**
	 * @var mediarepo_Core_AttributeDefinition definition of this attribute
	 *
	 * @access protected
	 */
	protected $_attrdef;
	...
	/**
	 * mediarepo_Core_Attribute constructor.
	 * @param $id
	 * @param $obj
	 * @param $attrdef
	 * @param $value
	 ...
	 * can be requested by mediarepo_Core_Attribute::getValidationError()
	 *
	 * @return boolean true if validation succeeds, otherwise false
	 */
	function validate() { /* {{{ */
		/** @var mediarepo_Core_AttributeDefinition $attrdef */
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_AttributeDefinition { /* {{{ */
	/**
	...
	/**
	 * @var mediarepo_Core_REPO reference to the repo instance this attribute definition belongs to
	 *
	 * @access protected
	 */
	protected $_repo;
	 ...
	 *
	 * Use {@link mediarepo_Core_AttributeDefinition::getValueSetAsArray()}
	...
	*/
	...
	function getStatistics($limit=0) { /* {{{ */
		...
		if($this->_objtype == mediarepo_Core_AttributeDefinition::objtype_all ||
		   $this->_objtype == mediarepo_Core_AttributeDefinition::objtype_document) {
			$queryStr = "SELECT * FROM `tblDocumentAttributes` WHERE `attrdef`=".$this->_id;
			if($limit)
			...
			if($resArr) {
				foreach($resArr as $rec) {
					if($doc = $this->_repo->getDocument($rec['document'])) {
						$result['docs'][] = $doc;
					}
				}
			}
			...
			if($resArr) {
				foreach($resArr as $row) {
					$tmpattr = new mediarepo_Core_Attribute(0, null, $this, $row['value']);
		...
		if($this->_objtype == mediarepo_Core_AttributeDefinition::objtype_all ||
		   $this->_objtype == mediarepo_Core_AttributeDefinition::objtype_folder) {
			$queryStr = "SELECT * FROM `tblFolderAttributes` WHERE `attrdef`=".$this->_id;
			if($limit)
				$queryStr .= " limit ".(int) $limit;
			$resArr = $db->getResultArray($queryStr);
			if($resArr) {
				foreach($resArr as $rec) {
					if($folder = $this->_repo->getFolder($rec['folder'])) {
		...
		if($this->_objtype == mediarepo_Core_AttributeDefinition::objtype_all ||
		   $this->_objtype == mediarepo_Core_AttributeDefinition::objtype_documentcontent) {
			$queryStr = "SELECT * FROM `tblDocumentContentAttributes` WHERE `attrdef`=".$this->_id;
			if($limit)
				$queryStr .= " limit ".(int) $limit;
			$resArr = $db->getResultArray($queryStr);
			if($resArr) {
				foreach($resArr as $rec) {
					if($content = $this->_repo->getDocumentContent($rec['content'])) {
	...
	public function removeValue($attrvalue) { /* {{{ */
		...
		foreach(array('document', 'documentcontent', 'folder') as $type) {
			if($type == 'document') {
				...
				$objtype = mediarepo_Core_AttributeDefinition::objtype_document;
			} elseif($type == 'documentcontent') {
				$tablename = "tblDocumentContentAttributes";
				$objtype = mediarepo_Core_AttributeDefinition::objtype_documentcontent;
			} elseif($type == 'folder') {
				$tablename = "tblFolderAttributes";
				$objtype = mediarepo_Core_AttributeDefinition::objtype_folder;
			}
			if($this->_objtype == mediarepo_Core_AttributeDefinition::objtype_all || $objtype) {
	...
	function validate($attrvalue) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onAttributeValidate'])) {
			foreach($this->_repo->callbacks['onAttributeValidate'] as $callback) {
