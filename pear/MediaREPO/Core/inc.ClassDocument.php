 ...
 *
 * A document in MediaREPO is similar to files in a regular file system.
 * Documents may have any number of content elements
 * ({@link mediarepo_Core_DocumentContent}). These content elements are often
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_Document extends mediarepo_Core_Object { /* {{{ */
	/**
	...
	/**
	 * @var mediarepo_Core_DocumentCategory[] list of categories
	 */
	...
	/**
	 * @var mediarepo_Core_DocumentContent temp. storage for latestcontent
	 */
	...
	/**
	 * @var mediarepo_Core_Folder
	 */
	protected $_folder;

	/** @var array of mediarepo_Core_UserAccess and mediarepo_Core_GroupAccess */
	 ...
	 */
	public static function getSearchFields($repo, $searchin) { /* {{{ */
		$db = $repo->getDB();
	 ...
	 * @param mediarepo_Core_REPO $repo
	 * @return bool|mediarepo_Core_Document instance of mediarepo_Core_Document if document exists, null
	 * if document does not exist, false in case of error
	 */
	public static function getInstance($id, $repo) { /* {{{ */
		$db = $repo->getDB();
		...
		$classname = $repo->getClassname('document');
		/** @var mediarepo_Core_Document $document */
	 ...
	 */
	function getDir() { /* {{{ */
		if($this->_repo->maxDirID) {
			$dirid = (int) (($this->_id-1) / $this->_repo->maxDirID) + 1;
	 ...
	 *
	 * @return bool|mediarepo_Core_DocumentCategory[]
	 */
	function getCategories() { /* {{{ */
		...
		if(!$this->_categories) {
			...
			foreach ($resArr as $row) {
				$cat = new mediarepo_Core_DocumentCategory($row['id'], $row['name']);
				$cat->setREPO($this->_repo);
	 ...
	 * categories.
	 *
	 * @param mediarepo_Core_DocumentCategory[] $newCategories list of category objects
	 * @return bool
	 */
	 ...
	 *
	 * @return mediarepo_Core_Folder parent folder
	 */
	function getParent() { /* {{{ */
	...
	function getFolder() { /* {{{ */
		if (!isset($this->_folder))
			$this->_folder = $this->_repo->getFolder($this->_folderID);
		return $this->_folder;
	} /* }}} */
	 ...
	 * folder.
	 *
	 * @param mediarepo_Core_Folder $newFolder
	...
	function setFolder($newFolder) { /* {{{ */
		...
		$flist = "";
		/** @var mediarepo_Core_Folder[] $path */
		foreach ($path as $f) {
			$flist .= ":".$f->getID();
		}
		if (strlen($flist)>1) {
	 ...
	 *
	 * @return mediarepo_Core_User owner of document as an instance of {@link mediarepo_Core_User}
	 */
	function getOwner() { /* {{{ */
	 ...
	 *
	 * @param mediarepo_Core_User $newOwner new owner
	 ...
	 *
	 * @param mediarepo_Core_User|bool $falseOrUser user object for locking or false for unlocking
	 * @return boolean true if operation was successful otherwise false
	 */
	 ...
	 *
	 * @return mediarepo_Core_User|bool user have a lock
	 */
	function getLockingUser() { /* {{{ */
		...
		if (!isset($this->_lockingUser))
			$this->_lockingUser = $this->_repo->getUser($this->_locked);
	 ...
	 * of {@link mediarepo_Core_UserAccess} and
	 * {@link mediarepo_Core_GroupAccess} objects. Even if the document
	...
	function getAccessList($mode = M_ANY, $op = O_EQ) { /* {{{ */
		...
		if (!isset($this->_accessList[$mode])) {
			...
			foreach ($resArr as $row) {
				if ($row["userID"] != -1)
					array_push($this->_accessList[$mode]["users"], new mediarepo_Core_UserAccess($this->_repo->getUser($row["userID"]), $row["mode"]));
				else //if ($row["groupID"] != -1)
					array_push($this->_accessList[$mode]["groups"], new mediarepo_Core_GroupAccess($this->_repo->getGroup($row["groupID"]), $row["mode"]));
			}
		}
	...
	function removeAccess($userOrGroupID, $isUser) { /* {{{ */
		...
		$mode = ($isUser ? $this->getAccessMode($this->_repo->getUser($userOrGroupID)) : $this->getGroupAccessMode($this->_repo->getGroup($userOrGroupID)));
		if ($mode == M_NONE) {
			$this->removeNotify($userOrGroupID, $isUser);
		}
	 ...
	 * {@link mediarepo_Core_DocumentContent::getAccessMode()} which calls
	 ...
	 *
	 * @param $user object instance of class mediarepo_Core_User
	...
	function getAccessMode($user) { /* {{{ */
		...
		/** @var mediarepo_Core_UserAccess $userAccess */
		...
		if($accessList['groups']) {
			$mode = 0;
			/** @var mediarepo_Core_GroupAccess $groupAccess */
	 ...
	 * For a list of possible access rights see @file inc.AccessUtils.php
	 *
	 * @param mediarepo_Core_Group $group object instance of class mediarepo_Core_Group
	 * @return integer access mode
	 */
	 ...
	 * is an array itself countaining objects of class mediarepo_Core_User and
	 * mediarepo_Core_Group.
	 *
	...
	function getNotifyList($type=0) { /* {{{ */
		if (empty($this->_notifyList)) {
			...
			{
				if ($row["userID"] != -1) {
					$u = $this->_repo->getUser($row["userID"]);
					if($u && !$u->isDisabled())
						array_push($this->_notifyList["users"], $u);
				} else { //if ($row["groupID"] != -1)
					$g = $this->_repo->getGroup($row["groupID"]);
					if($g)
	...
	function addNotify($userOrGroupID, $isUser) { /* {{{ */
		...
		$obj = ($isUser ? $this->_repo->getUser($userOrGroupID) : $this->_repo->getGroup($userOrGroupID));
		if (!is_object($obj)) {
			return -1;
		}
		...
		if ($isUser) {
			...
			if ($this->getDefaultAccess() >= M_READ) {
				...
				$found = false;
				/** @var mediarepo_Core_GroupAccess $group */
	...
	function removeNotify($userOrGroupID, $isUser, $type=0) { /* {{{ */
		...
		/* Verify that user / group exists. */
		/** @var mediarepo_Core_Group|mediarepo_Core_User $obj */
		$obj = ($isUser ? $this->_repo->getUser($userOrGroupID) : $this->_repo->getGroup($userOrGroupID));
		if (!is_object($obj)) {
			return -1;
		}
	 ...
	 * @return bool|mediarepo_Core_AddContentResultSet
	 */
	function addContent($comment, $user, $tmpFile, $orgFileName, $fileType, $mimeType, $reviewers=array(), $approvers=array(), $version=0, $attributes=array(), $workflow=null) { /* {{{ */
		...
		$filesize = mediarepo_Core_File::fileSize($tmpFile);
		$checksum = mediarepo_Core_File::checksum($tmpFile);
		...
		// copy file
		if (!mediarepo_Core_File::makeDir($this->_repo->contentDir . $dir)) {
			$db->rollbackTransaction();
			return false;
		}
		if($this->_repo->forceRename)
			$err = mediarepo_Core_File::renameFile($tmpFile, $this->_repo->contentDir . $dir . $version . $fileType);
		else
			$err = mediarepo_Core_File::copyFile($tmpFile, $this->_repo->contentDir . $dir . $version . $fileType);
		if (!$err) {
			$db->rollbackTransaction();
			return false;
		}
		...
		$docResultSet = new mediarepo_Core_AddContentResultSet($content);
		$docResultSet->setREPO($this->_repo);

		if($attributes) {
			foreach($attributes as $attrdefid=>$attribute) {
				/* $attribute can be a string or an array */
				if($attribute)
					if(!$content->setAttributeValue($this->_repo->getAttributeDefinition($attrdefid), $attribute)) {
		...
		// TODO - verify
		if ($this->_repo->enableConverting && in_array($docResultSet->getContent()->getFileType(), array_keys($this->_repo->convertFileTypes)))
		...
		foreach (array("i", "g") as $i){
			if (isset($reviewers[$i])) {
				foreach ($reviewers[$i] as $reviewerID) {
					$reviewer=($i=="i" ?$this->_repo->getUser($reviewerID) : $this->_repo->getGroup($reviewerID));
		...
		foreach (array("i", "g") as $i){
			if (isset($approvers[$i])) {
				foreach ($approvers[$i] as $approverID) {
					$approver=($i=="i" ? $this->_repo->getUser($approverID) : $this->_repo->getGroup($approverID));
	...
	function replaceContent($version, $user, $tmpFile, $orgFileName, $fileType, $mimeType) { /* {{{ */
		...
		// copy file
		if (!mediarepo_Core_File::copyFile($tmpFile, $this->_repo->contentDir . $dir . $version . $fileType)) {
			$db->rollbackTransaction();
			return false;
		}
	 ...
	 *
	 * @return bool|mediarepo_Core_DocumentContent[]
	 */
	function getContent() { /* {{{ */
		$db = $this->_repo->getDB();
		...
		if (!isset($this->_content)) {
			...
			$this->_content = array();
			$classname = $this->_repo->getClassname('documentcontent');
			$user = $this->_repo->getLoggedInUser();
			foreach ($resArr as $row) {
			    /** @var mediarepo_Core_DocumentContent $content */
	 ...
	 * @return mediarepo_Core_DocumentContent|boolean object of class {@link mediarepo_Core_DocumentContent}
	 * or false
	 */
	...
	/**
	 * @return bool|null|mediarepo_Core_DocumentContent
	 */
	 ...
	 * {@link mediarepo_Core_REPO::noReadForStatus} the function will go
	 ...
	 * currently logged in user.
	 *
	 * @return bool|mediarepo_Core_DocumentContent object of class {@link mediarepo_Core_DocumentContent}
	 */
	 ...
	 *
	 * @param mediarepo_Core_DocumentContent $version version number of content
	 * @return boolean true if successful, otherwise false
	 */
	private function _removeContent($version) { /* {{{ */
		$db = $this->_repo->getDB();

		if (file_exists( $this->_repo->contentDir.$version->getPath() ))
			if (!mediarepo_Core_File::removeFile( $this->_repo->contentDir.$version->getPath() ))
				return false;
		...
		foreach ($status as $st) {
			...
			foreach($resArr as $res) {
				$file = $this->_repo->contentDir . $this->getDir().'r'.$res['reviewLogID'];
				if(file_exists($file))
					mediarepo_Core_File::removeFile($file);
			}
		}
		...
		foreach ($status as $st) {
			...
			foreach($resArr as $res) {
				$file = $this->_repo->contentDir . $this->getDir().'a'.$res['approveLogID'];
				if(file_exists($file))
					mediarepo_Core_File::removeFile($file);
			}
		}
	...
	function removeContent($version) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPreRemoveContent'])) {
			foreach($this->_repo->callbacks['onPreRemoveContent'] as $callback) {
		...
		if(isset($this->_repo->callbacks['onPostRemoveContent'])) {
			foreach($this->_repo->callbacks['onPostRemoveContent'] as $callback) {
	 ...
	 * @return mediarepo_Core_DocumentLink|bool of mediarepo_Core_DocumentLink or false in case of
	 *         an error.
	 */
	function getDocumentLink($linkID) { /* {{{ */
		...
		$resArr = $resArr[0];
		$document = $this->_repo->getDocument($resArr["document"]);
		$target = $this->_repo->getDocument($resArr["target"]);
		$link = new mediarepo_Core_DocumentLink($resArr["id"], $document, $target, $resArr["userID"], $resArr["public"]);
		$user = $this->_repo->getLoggedInUser();
	 ...
	 * mediarepo_Core_REPO::filterDocumentLinks() afterwards.
	 *
	 * @param boolean $publiconly return on publically visible links
	 * @param object $user return also private links of this user
	 * @return array list of objects of class mediarepo_Core_DocumentLink
	 */
	function getDocumentLinks($publiconly=false, $user=null) { /* {{{ */
		if (!isset($this->_documentLinks)) {
			...
			$user = $this->_repo->getLoggedInUser();
			foreach ($resArr as $row) {
				$target = $this->_repo->getDocument($row["target"]);
				$link = new mediarepo_Core_DocumentLink($row["id"], $this, $target, $row["userID"], $row["public"]);
	 ...
	 * mediarepo_Core_Document::getDocumentLinks()
	 *
	...
	function getReverseDocumentLinks($publiconly=false, $user=null) { /* {{{ */
			...
			$links = array();
			foreach ($resArr as $row) {
				$document = $this->_repo->getDocument($row["document"]);
				$link = new mediarepo_Core_DocumentLink($row["id"], $document, $this, $row["userID"], $row["public"]);
	 ...
	 *
	 * @return object instance of mediarepo_Core_DocumentFile, null if file is not
	...
	function getDocumentFile($ID) { /* {{{ */
		...
		$resArr = $resArr[0];
		$file = new mediarepo_Core_DocumentFile($resArr["id"], $this, $resArr["userID"], $resArr["comment"], $resArr["date"], $resArr["dir"], $resArr["fileType"], $resArr["mimeType"], $resArr["orgFileName"], $resArr["name"],$resArr["version"],$resArr["public"]);
		$user = $this->_repo->getLoggedInUser();
	...
	function getDocumentFiles($version=0) { /* {{{ */
		if (!isset($this->_documentFiles)) {
			...
			$user = $this->_repo->getLoggedInUser();
			foreach ($resArr as $row) {
				$file = new mediarepo_Core_DocumentFile($row["id"], $this, $row["userID"], $row["comment"], $row["date"], $row["dir"], $row["fileType"], $row["mimeType"], $row["orgFileName"], $row["name"], $row["version"], $row["public"]);
				if($file->getAccessMode($user) >= M_READ)
	...
	function addDocumentFile($name, $comment, $user, $tmpFile, $orgFileName,$fileType, $mimeType,$version=0,$public=1) { /* {{{ */
		...
		// copy file
		if (!mediarepo_Core_File::makeDir($this->_repo->contentDir . $dir)) return false;
		if($this->_repo->forceRename)
			$err = mediarepo_Core_File::renameFile($tmpFile, $this->_repo->contentDir . $file->getPath());
		else
			$err = mediarepo_Core_File::copyFile($tmpFile, $this->_repo->contentDir . $file->getPath());
		if (!$err) return false;
	...
	function removeDocumentFile($ID) { /* {{{ */
		...
		if (file_exists( $this->_repo->contentDir . $file->getPath() )){
			if (!mediarepo_Core_File::removeFile( $this->_repo->contentDir . $file->getPath() ))
				return false;
		}
	...
	function remove() { /* {{{ */
		...
		/* Check if 'onPreRemoveDocument' callback is set */
		if(isset($this->_repo->callbacks['onPreRemoveDocument'])) {
			foreach($this->_repo->callbacks['onPreRemoveDocument'] as $callback) {
		...
		if (file_exists( $this->_repo->contentDir . $this->getDir() ))
			if (!mediarepo_Core_File::removeDir( $this->_repo->contentDir . $this->getDir() )) {
				$db->rollbackTransaction();
				return false;
			}
		...
		/* Check if 'onPostRemoveDocument' callback is set */
		if(isset($this->_repo->callbacks['onPostRemoveDocument'])) {
			foreach($this->_repo->callbacks['onPostRemoveDocument'] as $callback) {
	 ...
	 * {@see mediarepo_Core_Document::getReadAccessList()} instead.
	 */
			...
			if ($defAccess < M_READ) {
				if (strlen($groupIDs)>0) {
					$queryStr = "SELECT `tblUsers`.* FROM `tblUsers` ".
						...
						"AND `tblUsers`.`role` != ".mediarepo_Core_User::role_guest." UNION ";
				}
				$queryStr .=
					"SELECT `tblUsers`.* FROM `tblUsers` ".
					"WHERE (`tblUsers`.`role` != ".mediarepo_Core_User::role_guest.") ".
					"AND ((`tblUsers`.`id` = ". $this->_ownerID . ") ".
					"OR (`tblUsers`.`role` = ".mediarepo_Core_User::role_admin.")".
			 ...
			 */
				...
				if (strlen($groupIDs)>0) {
					$queryStr = "SELECT `tblUsers`.* FROM `tblUsers` ".
						...
						"AND `tblUsers`.`role` != ".mediarepo_Core_User::role_guest." ".
...
//					(strlen($userIDs) == 0 ? "" : " AND (`tblUsers`.`id` NOT IN (". $userIDs ."))").
			...
			if (!is_bool($resArr)) {
				foreach ($resArr as $row) {
					$user = $this->_repo->getUser($row['id']);
					if (!$listadmin && $user->isAdmin()) continue;
					if (!$listowner && $user->getID() == $this->_ownerID) continue;
			...
			if (strlen($queryStr)>0) {
				...
				if (!is_bool($resArr)) {
					foreach ($resArr as $row) {
						$group = $this->_repo->getGroup($row["id"]);
 ...
 * {@link mediarepo_Core_Document::removeContent()}.
 *
 * @category   REPO
 * @package    mediarepo_Core
 * @author     Markus Westphal, Malcolm Cowe, Matteo Lucarelli,
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_DocumentContent extends mediarepo_Core_Object { /* {{{ */
	 ...
	 * This method will call {@see mediarepo_Core_DocumentContent::setStatus()}
	...
	function __construct($id, $document, $version, $comment, $date, $userID, $dir, $orgFileName, $fileType, $mimeType, $fileSize=0, $checksum='') { /* {{{ */
		...
		$this->_repo = $document->_repo;
		if(!$fileSize) {
			$this->_fileSize = mediarepo_Core_File::fileSize($this->_repo->contentDir . $this->getPath());
		} else {
	...
	function getUser() { /* {{{ */
		if (!isset($this->_user))
			$this->_user = $this->_document->_repo->getUser($this->_userID);
		return $this->_user;
	} /* }}} */
	...
	function setFileSize() { /* {{{ */
		$filesize = mediarepo_Core_File::fileSize($this->_repo->contentDir . $this->_document->getDir() . $this->getFileName());
		if($filesize === false)
			return false;
	...
	function setChecksum() { /* {{{ */
		$checksum = mediarepo_Core_File::checksum($this->_repo->contentDir . $this->_document->getDir() . $this->getFileName());
		if($checksum === false)
			return false;
	 ...
	 */
	function convert() { /* {{{ */
		if (file_exists($this->_document->_repo->contentDir . $this->_document->getID() .'/' . "index.html"))
			return true;

		if (!in_array($this->_fileType, array_keys($this->_document->_repo->convertFileTypes)))
			return false;

		$source = $this->_document->_repo->contentDir . $this->_document->getID() .'/' . $this->getFileName();
		$target = $this->_document->_repo->contentDir . $this->_document->getID() .'/' . "index.html";
	//	$source = str_replace("/", "\\", $source);
	//	$target = str_replace("/", "\\", $target);

		$command = $this->_document->_repo->convertFileTypes[$this->_fileType];
	...
	/* FIXME: this function should not be part of the REPO. It lies in the duty
	 * of the application whether a file can be viewed online or not.
	 */
	function viewOnline() { /* {{{ */
		if (!isset($this->_document->_repo->_viewOnlineFileTypes) || !is_array($this->_document->_repo->_viewOnlineFileTypes)) {
			return false;
		}

		if (in_array(strtolower($this->_fileType), $this->_document->_repo->_viewOnlineFileTypes))
			return true;

		if ($this->_document->_repo->enableConverting && in_array($this->_fileType, array_keys($this->_document->_repo->convertFileTypes)))
	...
	function wasConverted() { /* {{{ */
		return file_exists($this->_document->_repo->contentDir . $this->_document->getID() .'/' . "index.html");
	} /* }}} */
	...
	function rewriteStatusLog($statuslog) { /* {{{ */
		...
		foreach($statuslog as $log) {
			if(!mediarepo_Core_REPO::checkDate($log['date'], 'Y-m-d H:i:s')) {
				$db->rollbackTransaction();
				return false;
			}
			$queryStr = "INSERT INTO `tblDocumentStatusLog` (`statusID`, `status`, `comment`, `date`, `userID`) ".
	 ...
	 * content. The function checks if {@link mediarepo_Core_REPO::noReadForStatus}
	 ...
	 * {@link mediarepo_Core_Document::getLatestContent()}
	 * It is also used by {@link mediarepo_Core_Document::getAccessMode()} to
	 ...
	 * FIXED: instead of using $repo->noReadForStatus it is take from the user's role
	 *
	 * @param object $u user
	 * @return integer either M_NONE or M_READ
	 */
	function getAccessMode($u) { /* {{{ */
		$repo = $this->_document->_repo;

		/* Check if 'onCheckAccessDocumentContent' callback is set */
		if(isset($this->_repo->callbacks['onCheckAccessDocumentContent'])) {
			foreach($this->_repo->callbacks['onCheckAccessDocumentContent'] as $callback) {
		...
		if(!$repo->noReadForStatus)
			return M_READ;
		$noReadForStatus = $repo->noReadForStatus;
			...
			foreach ($status as $r) {
				if($r['status'] != -2) // Check if reviewer was removed
					...
					case 1: // Reviewer is a group.
						$required = $repo->getGroup($r["required"]);
		...
		if (1 || !isset($this->_reviewStatus)) {
			...
			if($recs) {
				foreach($recs as $rec) {
					...
					foreach($res as &$t) {
						$filename = $this->_repo->contentDir . $this->_document->getDir().'r'.$t['reviewLogID'];
	...
	function rewriteReviewLog($reviewers) { /* {{{ */
		...
		foreach($reviewers as $review) {
			...
			foreach($reviewlog as $log) {
				...
				if(!empty($log['file'])) {
					mediarepo_Core_File::copyFile($log['file'], $this->_repo->contentDir . $this->_document->getDir() . 'r' . $reviewLogID);
				}
			}
		}
		...
		if (1 || !isset($this->_approvalStatus)) {
			...
			if($recs) {
				foreach($recs as $rec) {
					...
					foreach($res as &$t) {
						$filename = $this->_repo->contentDir . $this->_document->getDir().'a'.$t['approveLogID'];
						if(file_exists($filename))
							$t['file'] = $filename;
						else
							$t['file'] = '';
	...
	function rewriteApprovalLog($reviewers) { /* {{{ */
		...
		foreach($reviewers as $review) {
			...
			foreach($reviewlog as $log) {
				...
				if(!empty($log['file'])) {
					mediarepo_Core_File::copyFile($log['file'], $this->_repo->contentDir . $this->_document->getDir() . 'a' . $approveLogID);
				}
			}
		}
	 ...
	 *
	 * @see mediarepo_Core_DocumentContent::setApprovalByInd()
	...
	function setReviewByInd($user, $requestUser, $status, $comment, $file='') { /* {{{ */
		...
		$reviewLogID = $db->getInsertID('tblDocumentReviewLog', 'reviewLogID');
		if($file) {
			mediarepo_Core_File::copyFile($file, $this->_repo->contentDir . $this->_document->getDir() . 'r' . $reviewLogID);
		}
		return $reviewLogID;
 } /* }}} */
	 ...
	 * {@see mediarepo_Core_DocumentContent::setReviewByInd()} but adds a review
	 ...
	 *
	 * @see mediarepo_Core_DocumentContent::setReviewByInd()
	 ...
	 *       mediarepo_Core_DocumentContent::delIndApprover} instead)
	...
	function setApprovalByInd($user, $requestUser, $status, $comment, $file='') { /* {{{ */
		...
		$approveLogID = $db->getInsertID('tblDocumentApproveLog', 'approveLogID');
		if($file) {
			mediarepo_Core_File::copyFile($file, $this->_repo->contentDir . $this->_document->getDir() . 'a' . $approveLogID);
		}
		return $approveLogID;
 } /* }}} */
	 ...
	 * {link mediarepo_Core_DocumentContent::setApprovalByInd} but does it for
	 ...
	 *
	 * @return object/boolean an object of class mediarepo_Core_Workflow_State
	...
	function getWorkflowState() { /* {{{ */
		...
		if (!$this->_workflowState) {
			...
			if (is_bool($recs) && !$recs)
				return false;
			$this->_workflowState = new mediarepo_Core_Workflow_State($recs[0]['id'], $recs[0]['name'], $recs[0]['maxtime'], $recs[0]['precondfunc'], $recs[0]['documentstatus']);
			$this->_workflowState->setREPO($this->_document->_repo);
		}
	 ...
	 *
	 * @return object/boolean an object of class mediarepo_Core_Workflow
	 *         or false in case of error, e.g. the version has not a workflow
	 */
	function getWorkflow() { /* {{{ */
		...
		if (!isset($this->_workflow)) {
			...
			if(!$recs)
				return false;
			$this->_workflow = new mediarepo_Core_Workflow($recs[0]['id'], $recs[0]['name'], $this->_document->_repo->getWorkflowState($recs[0]['initstate']));
			$this->_workflow->setREPO($this->_document->_repo);
		}
	 ...
	 *        when it has reached a valid end state
	          (see mediarepo_Core_DocumentContent::enterNextState())
	...
	function removeWorkflow($user, $unlink=false) { /* {{{ */
		...
		if(mediarepo_Core_REPO::checkIfEqual($this->_workflow->getInitState(), $this->getWorkflowState()) || $unlink == true) {
			$db->startTransaction();
			$queryStr=
	...
	function getParentWorkflow() { /* {{{ */
		...
		if($recs[0]['parentworkflow'])
			return $this->_document->_repo->getWorkflow($recs[0]['parentworkflow']);
	...
	function returnFromSubWorkflow($user, $transition=null, $comment='') { /* {{{ */
		...
		if (isset($this->_workflow)) {
			...
			$this->_workflow = $this->_document->_repo->getWorkflow($recs[0]['parentworkflow']);
			$this->_workflow->setREPO($this->_document->_repo);
...
*/
		...
		for ($i = 0; $i < count($resArr); $i++) {
			$workflow = $this->_document->_repo->getWorkflow($resArr[$i]["workflow"]);
			$workflowlog = new mediarepo_Core_Workflow_Log($resArr[$i]["id"], $this->_document->_repo->getDocument($resArr[$i]["document"]), $resArr[$i]["version"], $workflow, $this->_document->_repo->getUser($resArr[$i]["userid"]), $workflow->getTransition($resArr[$i]["transition"]), $resArr[$i]["date"], $resArr[$i]["comment"]);
			$workflowlog->setREPO($this);
	...
	function getLastWorkflowTransition() { /* {{{ */
		...
		$workflowlogs = array();
		$i = 0;
		$workflowlog = new mediarepo_Core_Workflow_Log($resArr[$i]["id"], $this->_document->_repo->getDocument($resArr[$i]["document"]), $resArr[$i]["version"], $this->_workflow, $this->_document->_repo->getUser($resArr[$i]["userid"]), $this->_workflow->getTransition($resArr[$i]["transition"]), $resArr[$i]["date"], $resArr[$i]["comment"]);
		$workflowlog->setREPO($this);
 ...
 * Use {@link mediarepo_Core_Document::addDocumentLink()} to add a reference
 * to another document.
 *
 * @category   REPO
 * @package    mediarepo_Core
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_DocumentLink { /* {{{ */
	/**
	...
	/**
	 * @var mediarepo_Core_Document reference to document this link belongs to
	 */
	protected $_document;
	...
	/**
	 * mediarepo_Core_DocumentLink constructor.
	 * @param $id
	 * @param $document
	 * @param $target
	 * @param $userID
	...
	/**
	 * @return mediarepo_Core_Document
	 */
	 ...
	 *
	 * @param mediarepo_Core_User $u user
	 * @param $source
	 * @param $target
	...
	function getAccessMode($u, $source, $target) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onCheckAccessDocumentLink'])) {
			foreach($this->_repo->callbacks['onCheckAccessDocumentLink'] as $callback) {
 ...
 * Use {@link mediarepo_Core_Document::addDocumentFile()} to attach a file.
 *
 * @category   REPO
 * @package    mediarepo_Core
 * @author     Markus Westphal, Malcolm Cowe, Matteo Lucarelli,
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_DocumentFile { /* {{{ */
	/**
	...
	/**
	 * @var mediarepo_Core_Document reference to document this file belongs to
	 */
	protected $_document;
	...
	/**
	 * mediarepo_Core_DocumentFile constructor.
	 * @param $id
	 * @param $document
	 * @param $userID
	 * @param $comment
	...
	function getAccessMode($u) { /* {{{ */
		...
		/* Check if 'onCheckAccessDocumentLink' callback is set */
		if(isset($this->_repo->callbacks['onCheckAccessDocumentFile'])) {
			foreach($this->_repo->callbacks['onCheckAccessDocumentFile'] as $callback) {
				if(($ret = call_user_func($callback[0], $callback[1], $this, $u)) > 0) {
					return $ret;
				}
			}
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_AddContentResultSet { /* {{{ */
	...
	/**
	 * mediarepo_Core_AddContentResultSet constructor.
	 * @param $content
	 */
	...
	function addReviewer($reviewer, $type, $status) { /* {{{ */
		...
		if (!strcasecmp($type, "i")) {
			if (strcasecmp(get_class($reviewer), $repo->getClassname("user"))) {
				return false;
			}
		...
		if (!strcasecmp($type, "g")) {
			if (strcasecmp(get_class($reviewer), $repo->getClassname("group"))) {
				return false;
			}
	...
	function addApprover($approver, $type, $status) { /* {{{ */
		...
		if (!strcasecmp($type, "i")) {
			if (strcasecmp(get_class($approver), $repo->getClassname("user"))) {
				return false;
			}
		...
		if (!strcasecmp($type, "g")) {
			if (strcasecmp(get_class($approver), $repo->getClassname("group"))) {
				return false;
			}
