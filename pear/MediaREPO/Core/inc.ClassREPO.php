 ...
 * This class is needed to do most of the repo operations. It needs
 * an instance of {@link mediarepo_Core_DatabaseAccess} to access the
 * underlying database. Many methods are factory functions which create
 * objects representing the entities in the repo, like folders, documents,
 * users, or groups.
 *
 * Each repo has its own database for meta data and a data store for document
 ...
 * {@link mediarepo_Core_Folder::getAccessMode()} and
 * {@link mediarepo_Core_Document::getAccessMode()} and interpret them as desired.
 ...
 * there is currently no class within the MediaREPO core which needs the logged
 * in user. {@link mediarepo_Core_REPO} itself does not do any user authentication.
 * It is up to the application using this class.
 *
 * <code>
 * <?php
 * include("inc/inc.ClassREPO.php");
 * $db = new mediarepo_Core_DatabaseAccess($type, $hostname, $user, $passwd, $name);
 * $db->connect() or die ("Could not connect to db-server");
 * $repo = new mediarepo_Core_REPO($db, $contentDir);
 * $repo->setRootFolderID(1);
 * ...
 * ?>
 * </code>
 *
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_REPO {
	/**
	 * @var mediarepo_Core_DatabaseAccess $db reference to database object. This must be an instance
	 *      of {@link mediarepo_Core_DatabaseAccess}.
	 * @access protected
	 */
	protected $db;
	...
	/**
	 * @var mediarepo_Core_User $user reference to currently logged in user. This must be
	 *      an instance of {@link mediarepo_Core_User}. This variable is currently not
	 ...
	 *      mediarepo_Core_Document::addDocument() and
	 *      mediarepo_Core_Document::addDocumentFile(). Setting this to true
	 ...
	 * {@link mediarepo_Core_REPO::setCallback()}.
	 ...
	 *
	 * @param mediarepo_Core_DocumentLink[] $links list of objects of type mediarepo_Core_DocumentLink
	 ...
	 * owned by the user, or the accessing user is an administrator.
	 *
	 * @param array $files list of objects of type mediarepo_Core_DocumentFile
	 * @param object $user user for which access is being checked
	 ...
	 *
	 * @param mediarepo_Core_DatabaseAccess $db object of class {@link mediarepo_Core_DatabaseAccess}
	...
	function __construct($db, $contentDir) { /* {{{ */
		...
		$this->classnames['folder'] = 'mediarepo_Core_Folder';
		$this->classnames['document'] = 'mediarepo_Core_Document';
		$this->classnames['documentcontent'] = 'mediarepo_Core_DocumentContent';
		$this->classnames['user'] = 'mediarepo_Core_User';
		$this->classnames['group'] = 'mediarepo_Core_Group';
	 ...
	 * by the repo. Each class has an internal place holder, which must be
	 * passed to function.
	 *
	 ...
	 * by the repo. It is mainly used to create a new class (possible
	 * inherited from one of the available classes) implementing new
	 * features. The method should be called in the postInitREPO hook.
	 *
	 * @param string $objectname placeholder (can be one of 'folder', 'document',
	 * 'documentcontent', 'user', 'group'
	 ...
	 *
	 * @return mediarepo_Core_DatabaseAccess database
	 */
	function getDB() { /* {{{ */
		return $this->db;
	} /* }}} */
	 ...
	 * MediaREPO an additional directory level has been introduced, which
	 ...
	 * Get root folder
	 *
	 * @return mediarepo_Core_Folder|boolean return the object of the root folder or false if
	 ...
	 * be used to tell the repo who is currently logged in.
	 *
	 * @param object $user
	 *
	 */
	 ...
	 * be used to tell the repo who is currently logged in.
	 *
	 * @return mediarepo_Core_User $user
	 *
	 */
	 ...
	 * @return mediarepo_Core_Document instance of {@link mediarepo_Core_Document} or false
	 */
	 ...
	 * @return bool|mediarepo_Core_Document[]
	 */
	function getDocumentsExpired($date, $user=null) { /* {{{ */
		...
		/** @var mediarepo_Core_Document[] $documents */
		$documents = array();
	 ...
	 * @param string $name
	 * @param object $folder
	 * @return mediarepo_Core_Document|boolean found document or false
	 */
	 ...
	 * @return bool|mediarepo_Core_Document or false
	 */
	 ...
	 * @param mediarepo_Core_User $param1 user
...
//				"ORDER BY `statusDate` DESC";
	 ...
	 * @param mediarepo_Core_Folder|null $startFolder search in the folder only (null for root folder)
	 * @param mediarepo_Core_User $owner search for documents owned by this user
	...
	function search($query, $limit=0, $offset=0, $logicalmode='AND', $searchin=array(), $startFolder=null, $owner=null, $status = array(), $creationstartdate=array(), $creationenddate=array(), $modificationstartdate=array(), $modificationenddate=array(), $categories=array(), $attributes=array(), $mode=0x3, $expirationstartdate=array(), $expirationenddate=array()) { /* {{{ */
		...
		if($mode & 0x2) {
			...
			if ($attributes) {
				foreach($attributes as $attrdefid=>$attribute) {
					if($attribute) {
						...
						if($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_folder || $attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_all) {
			...
			if ($creationstartdate) {
				$startdate = mediarepo_Core_REPO::makeTimeStamp($creationstartdate['hour'], $creationstartdate['minute'], $creationstartdate['second'], $creationstartdate['year'], $creationstartdate["month"], $creationstartdate["day"]);
				if ($startdate) {
					$searchCreateDate .= "`tblFolders`.`date` >= ".$startdate;
				}
			}
			if ($creationenddate) {
				$stopdate = mediarepo_Core_REPO::makeTimeStamp($creationenddate['hour'], $creationstartdate['minute'], $creationstartdate['second'], $creationenddate["year"], $creationenddate["month"], $creationenddate["day"]);
				if ($stopdate) {
		...
		if($mode & 0x1) {
			...
			if ($attributes) {
				foreach($attributes as $attrdefid=>$attribute) {
					if($attribute) {
						$attrdef = $this->getAttributeDefinition($attrdefid);
						if($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_document || $attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_all) {
							if($valueset = $attrdef->getValueSet()) {
								if($attrdef->getMultipleValues()) {
									if(is_string($attribute))
										$attribute = array($attribute);
						...
						} elseif($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_documentcontent) {
			...
			if ($creationenddate) {
				$stopdate = mediarepo_Core_REPO::makeTimeStamp($creationenddate['hour'], $creationenddate['minute'], $creationenddate['second'], $creationenddate["year"], $creationenddate["month"], $creationenddate["day"]);
				if ($stopdate) {
					if($searchCreateDate)
						$searchCreateDate .= " AND ";
			...
			if ($modificationstartdate) {
				$startdate = mediarepo_Core_REPO::makeTimeStamp($modificationstartdate['hour'], $modificationstartdate['minute'], $modificationstartdate['second'], $modificationstartdate['year'], $modificationstartdate["month"], $modificationstartdate["day"]);
				if ($startdate) {
					if($searchCreateDate)
						$searchCreateDate .= " AND ";
			...
			if ($modificationenddate) {
				$stopdate = mediarepo_Core_REPO::makeTimeStamp($modificationenddate['hour'], $modificationenddate['minute'], $modificationenddate['second'], $modificationenddate["year"], $modificationenddate["month"], $modificationenddate["day"]);
				if ($stopdate) {
					if($searchCreateDate)
						$searchCreateDate .= " AND ";
			...
			if ($expirationstartdate) {
				$startdate = mediarepo_Core_REPO::makeTimeStamp($expirationstartdate['hour'], $expirationstartdate['minute'], $expirationstartdate['second'], $expirationstartdate['year'], $expirationstartdate["month"], $expirationstartdate["day"]);
				if ($startdate) {
			...
			if ($expirationenddate) {
				$stopdate = mediarepo_Core_REPO::makeTimeStamp($expirationenddate['hour'], $expirationenddate['minute'], $expirationenddate['second'], $expirationenddate["year"], $expirationenddate["month"], $expirationenddate["day"]);
				if ($stopdate) {
					if($searchExpirationDate)
						$searchExpirationDate .= " AND ";
	 ...
	 * @return mediarepo_Core_Folder instance of mediarepo_Core_Folder or false
	 */
	 ...
	 * @param mediarepo_Core_Folder $folder parent folder
	 * @return mediarepo_Core_Folder|boolean found folder or false
	 */
	function getFolderByName($name, $folder=null) { /* {{{ */
		...
		$resArr = $resArr[0];
		/** @var mediarepo_Core_Folder $folder */
	 ...
	 * @return mediarepo_Core_User|boolean instance of {@link mediarepo_Core_User} or false
	 */
	function getUser($id) { /* {{{ */
	 ...
	 * @return object instance of {@link mediarepo_Core_User} or false
	 */
	 ...
	 * @return array of instances of <a href='psi_element://mediarepo_Core_User'>mediarepo_Core_User</a> or false
	 * or false
	 */
	...
	function addUser($login, $pwd, $fullName, $email, $language, $theme, $comment, $role='0', $isHidden=0, $isDisabled=0, $pwdexpiration='', $quota=0, $homefolder=null) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPostAddUser'])) {
			foreach($this->_repo->callbacks['onPostUser'] as $callback) {
	 ...
	 * @return mediarepo_Core_Group|boolean group or false if no group was found
	 */
	 ...
	 *
	 * @return mediarepo_Core_Group[] array of instances of {@link mediarepo_Core_Group}
	 */
	 ...
	 * @return mediarepo_Core_Group|boolean instance of {@link mediarepo_Core_Group} or false in
	 *         case of an error.
	 */
	function addGroup($name, $comment) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPostAddGroup'])) {
			foreach($this->_repo->callbacks['onPostAddGroup'] as $callback) {
				/** @noinspection PhpStatementHasEmptyBodyInspection */
	...
	function getKeywordCategory($id) { /* {{{ */
		...
		$resArr = $resArr[0];
		$cat = new mediarepo_Core_Keywordcategory($resArr["id"], $resArr["owner"], $resArr["name"]);
		$cat->setREPO($this);
		return $cat;
	} /* }}} */
	...
	function getAllKeywordCategories($userIDs = array()) { /* {{{ */
		...
		$categories = array();
		foreach ($resArr as $row) {
			$cat = new mediarepo_Core_KeywordCategory($row["id"], $row["owner"], $row["name"]);
			$cat->setREPO($this);
	 ...
	 * @param $userID
	 * @return mediarepo_Core_KeywordCategory[]|bool
	 */
	...
	function addKeywordCategory($userID, $name) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPostAddKeywordCategory'])) {
			foreach($this->_repo->callbacks['onPostAddKeywordCategory'] as $callback) {
				/** @noinspection PhpStatementHasEmptyBodyInspection */
	...
	function getDocumentCategory($id) { /* {{{ */
		...
		$resArr = $resArr[0];
		$cat = new mediarepo_Core_DocumentCategory($resArr["id"], $resArr["name"]);
		$cat->setREPO($this);
		return $cat;
	} /* }}} */
	 ...
	 * @return mediarepo_Core_DocumentCategory|boolean instance of {@link mediarepo_Core_DocumentCategory}
	 */
	...
	function addDocumentCategory($name) { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPostAddDocumentCategory'])) {
			foreach($this->_repo->callbacks['onPostAddDocumentCategory'] as $callback) {
				/** @noinspection PhpStatementHasEmptyBodyInspection */
				if(!call_user_func($callback[0], $callback[1], $category)) {
				}
			}
	 ...
	 *
	 * deprecated: User {@link mediarepo_Core_Group::getNotifications()}
	 *
	 ...
	 *
	 * deprecated: User {@link mediarepo_Core_User::getNotifications()}
	 *
	 ...
	 * @return bool|mediarepo_Core_AttributeDefinition or false
	 */
	function getAttributeDefinition($id) { /* {{{ */
		...
		$attrdef = new mediarepo_Core_AttributeDefinition($resArr["id"], $resArr["name"], $resArr["objtype"], $resArr["type"], $resArr["multiple"], $resArr["minvalues"], $resArr["maxvalues"], $resArr["valueset"], $resArr["regex"]);
		$attrdef->setREPO($this);
		return $attrdef;
	} /* }}} */
	 ...
	 * @return mediarepo_Core_AttributeDefinition|boolean instance of {@link mediarepo_Core_AttributeDefinition} or false
	 */
	 ...
	 * @return bool|mediarepo_Core_AttributeDefinition[] of instances of <a href='psi_element://mediarepo_Core_AttributeDefinition'>mediarepo_Core_AttributeDefinition</a> or false
	 * or false
	 */
	function getAllAttributeDefinitions($objtype=0) { /* {{{ */
		...
		/** @var mediarepo_Core_AttributeDefinition[] $attrdefs */
		$attrdefs = array();

		for ($i = 0; $i < count($resArr); $i++) {
			$attrdef = new mediarepo_Core_AttributeDefinition($resArr[$i]["id"], $resArr[$i]["name"], $resArr[$i]["objtype"], $resArr[$i]["type"], $resArr[$i]["multiple"], $resArr[$i]["minvalues"], $resArr[$i]["maxvalues"], $resArr[$i]["valueset"], $resArr[$i]["regex"]);
			$attrdef->setREPO($this);
			$attrdefs[$i] = $attrdef;
		}
	 ...
	 *
	 * @return mediarepo_Core_Workflow[]|bool of instances of {@link mediarepo_Core_Workflow} or false
	 */
	function getAllWorkflows() { /* {{{ */
		...
		for ($i = 0; $i < count($ressArr); $i++) {
			$wkfstates[$ressArr[$i]["id"]] = new mediarepo_Core_Workflow_State($ressArr[$i]["id"], $ressArr[$i]["name"], $ressArr[$i]["maxtime"], $ressArr[$i]["precondfunc"], $ressArr[$i]["documentstatus"]);
		}

		/** @var mediarepo_Core_Workflow[] $workflows */
		$workflows = array();
		for ($i = 0; $i < count($resArr); $i++) {
			/** @noinspection PhpUndefinedVariableInspection */
			$workflow = new mediarepo_Core_Workflow($resArr[$i]["id"], $resArr[$i]["name"], $wkfstates[$resArr[$i]["initstate"]]);
			$workflow->setREPO($this);
	...
	function getWorkflow($id) { /* {{{ */
		...
		$workflow = new mediarepo_Core_Workflow($resArr[0]["id"], $resArr[0]["name"], $initstate);
		$workflow->setREPO($this);
	 ...
	 *
	 * @param string $name name of workflow
	 * @param mediarepo_Core_Workflow_State $initstate initial state of workflow
	 * @return bool|mediarepo_Core_Workflow
	 */
	 ...
	 * @return bool|mediarepo_Core_Workflow_State or false
	 */
	 ...
	 *
	 * @return mediarepo_Core_Workflow_State[]|bool of instances of {@link mediarepo_Core_Workflow_State} or false
	 */
	function getAllWorkflowStates() { /* {{{ */
		...
		$wkfstates = array();
		for ($i = 0; $i < count($ressArr); $i++) {
			$wkfstate = new mediarepo_Core_Workflow_State($ressArr[$i]["id"], $ressArr[$i]["name"], $ressArr[$i]["maxtime"], $ressArr[$i]["precondfunc"], $ressArr[$i]["documentstatus"]);
			$wkfstate->setREPO($this);
	 ...
	 * @return bool|mediarepo_Core_Workflow_State
	 */
	 ...
	 * @return mediarepo_Core_Workflow_Action|bool instance of {@link mediarepo_Core_Workflow_Action} or false
	 */
	function getWorkflowAction($id) { /* {{{ */
		...
		$action = new mediarepo_Core_Workflow_Action($resArr["id"], $resArr["name"]);
		$action->setREPO($this);
		return $action;
	} /* }}} */
	 ...
	 *
	 * @return mediarepo_Core_Workflow_Action[]|bool list of instances of {@link mediarepo_Core_Workflow_Action} or false
	 */
	function getAllWorkflowActions() { /* {{{ */
		...
		/** @var mediarepo_Core_Workflow_Action[] $wkfactions */
		$wkfactions = array();
		for ($i = 0; $i < count($resArr); $i++) {
			$action = new mediarepo_Core_Workflow_Action($resArr[$i]["id"], $resArr[$i]["name"]);
			$action->setREPO($this);
			$wkfactions[$i] = $action;
		}
	 ...
	 *
	 * @param string $name name of workflow action
	 * @return mediarepo_Core_Workflow_Action|bool
	 */
	 ...
	 * @return mediarepo_Core_Workflow_Transition|bool instance of {@link mediarepo_Core_Workflow_Transition} or false
	 */
	function getWorkflowTransition($id) { /* {{{ */
		...
		$transition = new mediarepo_Core_Workflow_Transition($resArr["id"], $this->getWorkflow($resArr["workflow"]), $this->getWorkflowState($resArr["state"]), $this->getWorkflowAction($resArr["action"]), $this->getWorkflowState($resArr["nextstate"]), $resArr["maxtime"]);
		$transition->setREPO($this);
		return $transition;
	} /* }}} */
	 ...
	 * in version 4.0.0 of MediaREPO for implementation of user quotas.
	 *
	 * @return mediarepo_Core_Document[]|bool
	 */
	function getNoFileSizeDocumentContent() { /* {{{ */
		...
		/** @var mediarepo_Core_Document[] $versions */
		$versions = array();
		foreach($resArr as $row) {
			/** @var mediarepo_Core_Document $document */
			$document = new $this->classnames['document']($row['document'], '', '', '', '', '', '', '', '', '', '', '');
	 ...
	 * in version 4.0.0 of MediaREPO for finding duplicates.
	 * @return bool|mediarepo_Core_Document[]
	 */
	...
	function getTimeline($startts='', $endts='') { /* {{{ */
		...
		/** @var mediarepo_Core_Document[] $timeline */
		$timeline = array();
