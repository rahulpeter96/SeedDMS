...
class mediarepo_Core_User { /* {{{ */
	...
	/**
	 * @var string role of user. Can be one of mediarepo_Core_User::role_user,
	 *      mediarepo_Core_User::role_admin, mediarepo_Core_User::role_guest
	 *
	 * @access protected
	 */
	var $_role;
	...
	/**
	 * @var mediarepo_Core_Folder home folder
	 *
	 * @access protected
	 */
	var $_homeFolder;
	...
	/**
	 * @var mediarepo_Core_REPO reference to the repo instance this user belongs to
	 *
	 * @access protected
	 */
	var $_repo;
	...
	/**
	 * mediarepo_Core_User constructor.
	 * @param $id
	 * @param $login
	 * @param $pwd
	 * @param $fullName
	 ...
	 * @param mediarepo_Core_REPO $repo instance of repo
	 ...
	 * @return mediarepo_Core_User|bool instance of class mediarepo_Core_User if user was
	 * found, null if user was not found, false in case of error
	 */
	public static function getInstance($id, $repo, $by='', $email='') { /* {{{ */
		$db = $repo->getDB();
	 ...
	 * @return mediarepo_Core_User[]|bool
	 */
	public static function getAllInstances($orderby, $repo) { /* {{{ */
		$db = $repo->getDB();
	 ...
	 * @return bool
	 */
	function isAdmin() { return ($this->_role == mediarepo_Core_User::role_admin); }
	...
	function setAdmin() { /* {{{ */
		...
		$queryStr = "UPDATE `tblUsers` SET `role` = " . mediarepo_Core_User::role_admin . " WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;

		$this->_role = mediarepo_Core_User::role_admin;
		return true;
	} /* }}} */
	 ...
	 * @return bool
	 */
	function isGuest() { return ($this->_role == mediarepo_Core_User::role_guest); }
	...
	function setGuest() { /* {{{ */
		...
		$queryStr = "UPDATE `tblUsers` SET `role` = " . mediarepo_Core_User::role_guest . " WHERE `id` = " . $this->_id;
		if (!$db->getResult($queryStr))
			return false;

		$this->_role = mediarepo_Core_User::role_guest;
		return true;
	} /* }}} */
	...
	/**
	 * @return null|mediarepo_Core_Folder
	 */
	 ...
	 * to a different user.
	 *
	 * @param mediarepo_Core_User $user the user doing the removal (needed for entry in
	 *        review and approve log).
	 * @param mediarepo_Core_User $assignToUser the user who is new owner of folders and
	 *        documents which previously were owned by the delete user.
	 * @return boolean true on success or false in case of an error
	 */
	 ...
	 * This function uses {@link mediarepo_Group::addUser} but checks before if
	 * the user is already a member of the group.
	 *
	 * @param mediarepo_Core_Group $group group to be the member of
	 ...
	 * This function uses {@link mediarepo_Group::removeUser} but checks before if
	 * the user is a member of the group at all.
	 *
	 * @param mediarepo_Core_Group $group group to leave
	 * @return boolean true on success or false in case of an error or the user
	 ...
	 *
	 * @return mediarepo_Core_Group[]|bool list of groups
	 */
	function getGroups() { /* {{{ */
		$db = $this->_repo->getDB();
		...
		if (!isset($this->_groups))
		{
			...
			$this->_groups = array();
			$classname = $this->_repo->getClassname('group');
			foreach ($resArr as $row) {
				/** @var mediarepo_Core_Group $group */
	 ...
	 *
	 * @param mediarepo_Core_Group $group
	 ...
	 * @return mediarepo_Core_Document[]|bool list of documents
	 */
	function getDocuments() { /* {{{ */
		...
		$documents = array();
		$classname = $this->_repo->getClassname('document');
		foreach ($resArr as $row) {
	 ...
	 *
	 * @return bool|mediarepo_Core_Document[] list of documents
	 */
	 ...
	 * {link mediarepo_Core_User::getApprovalStatus} which does the same for
	 * approvals.
	 *
	...
	function getWorkflowsInvolved() { /* {{{ */
		...
		if (count($resArr)>0) {
			foreach ($resArr as $res) {
				$result[] = $this->_repo->getWorkflow($res['id']);
			}
		}
	 ...
	 * See {link mediarepo_Core_User::getMandatoryReviewers}
	 *
	 * @return array list of arrays with two elements containing the user id
	 ...
	 *
	 * @return mediarepo_Core_User[]|bool list of users where this user is a mandatory reviewer.
	 */
	function isMandatoryReviewerOf() { /* {{{ */
		...
		$users = array();
		foreach($resArr as $res) {
			$users[] = self::getInstance($res['userID'], $this->_repo);
		}
	 ...
	 *
	 * @return mediarepo_Core_User[]|bool list of users where this user is a mandatory approver.
	 */
	 ...
	 *
	 * @return mediarepo_Core_Workflow|bool workflow
	 */
	function getMandatoryWorkflow() { /* {{{ */
		...
		$workflow = $this->_repo->getWorkflow($resArr[0]['workflow']);
		return $workflow;
	} /* }}} */
	...
	function getMandatoryWorkflows() { /* {{{ */
		...
		$workflows = array();
		foreach($resArr as $res) {
			$workflows[] = $this->_repo->getWorkflow($res['workflow']);
		}
		return $workflows;
	} /* }}} */
	 ...
	 *
	 * @param mediarepo_Core_Workflow[] $workflows list of workflow objects
	 * @return boolean true on success, otherwise false
	 */
	 ...
	 * @return mediarepo_Core_Notification[]|bool array of notifications
	 */
	function getNotifications($type=0) { /* {{{ */
		...
		$notifications = array();
		foreach ($resArr as $row) {
			$not = new mediarepo_Core_Notification($row["target"], $row["targetType"], $row["userID"], $row["groupID"]);
			$not->setREPO($this);
	 ...
	 *
	 * @return mediarepo_Core_KeywordCategory[]|bool list of categories or false in case of an error
	 */
