...
class mediarepo_Core_Group { /* {{{ */
	...
	/**
	 * Back reference to REPO this user group belongs to
	 *
	 ...
	 * @return mediarepo_Core_Group|bool instance of class mediarepo_Core_Group if group was
	 * found, null if group was not found, false in case of error
	 */
	public static function getInstance($id, $repo, $by='') { /* {{{ */
		$db = $repo->getDB();
	...
	function getUsers() { /* {{{ */
		...
		if (!isset($this->_users)) {
			...
			$classname = $this->_repo->getClassname('user');
	 ...
	 *
	 * @param mediarepo_Core_User $user user to be checked
