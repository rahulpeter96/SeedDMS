 ...
 *
 * A folder in MediaREPO is equivalent to a directory in a regular file
 ...
 * @version    Release: 5.1.7
 */
class mediarepo_Core_Folder extends mediarepo_Core_Object {
	/**
	...
	/**
	 * @var mediarepo_Core_Document[]
	 */
	protected $_documents;

	/**
	 * @var mediarepo_Core_UserAccess[]|mediarepo_Core_GroupAccess[]
	 */
	protected $_accessList;

	/**
	 * mediarepo_Core_Folder constructor.
	 * @param $id
	 * @param $name
	 * @param $parentID
	 * @param $comment
	 ...
	 * @param mediarepo_Core_REPO $repo
	 * @return mediarepo_Core_Folder|bool instance of mediarepo_Core_Folder if document exists, null
	...
	public static function getInstance($id, $repo) { /* {{{ */
		...
		$resArr = $resArr[0];
		$classname = $repo->getClassname('folder');
	 ...
	 * Returns the parent
	 *
	 * @return bool|mediarepo_Core_Folder
	 */
	public function getParent() { /* {{{ */
		if ($this->_id == $this->_repo->rootFolderID || empty($this->_parentID)) {
			return false;
		}

		if (!isset($this->_parent)) {
			$this->_parent = $this->_repo->getFolder($this->_parentID);
		}
		return $this->_parent;
	} /* }}} */
	 ...
	 * folder.
	 *
	 * @param mediarepo_Core_Folder $subfolder
	 ...
	 *
	 * @param mediarepo_Core_Folder $newParent new parent folder
	 ...
	 * Set the owner
	 *
	 * @param mediarepo_Core_User $newOwner of the folder
	 ...
	 * {@link mediarepo_Core_REPO::filterAccess} for checking each folder against
	 ...
	 * @return mediarepo_Core_Folder[]|bool list of folder objects or false in case of an error
	 */
	function getSubFolders($orderby="", $dir="asc", $limit=0, $offset=0) { /* {{{ */
		...
		if (!isset($this->_subFolders)) {
			...
			for ($i = 0; $i < count($resArr); $i++)
				$this->_subFolders[$i] = $this->_repo->getFolder($resArr[$i]["id"]);
		}
	...
	function addSubFolder($name, $comment, $owner, $sequence, $attributes=array()) { /* {{{ */
		...
		$newFolder = $this->_repo->getFolder($db->getInsertID('tblFolders'));
		unset($this->_subFolders);

		if($attributes) {
			foreach($attributes as $attrdefid=>$attribute) {
				if($attribute)
					if(!$newFolder->setAttributeValue($this->_repo->getAttributeDefinition($attrdefid), $attribute)) {
						$db->rollbackTransaction();
						return false;
					}
			}
		...
		if(isset($this->_repo->callbacks['onPostAddSubFolder'])) {
			foreach($this->_repo->callbacks['onPostAddSubFolder'] as $callback) {
	 ...
	 * This function does not check for access rights. Use
	 * {@link mediarepo_Core_REPO::filterAccess} for checking each document against
	 * the currently logged in user and the access rights.
	 *
	 * @param string $orderby if set to 'n' the list is ordered by name, otherwise
	 *        it will be ordered by sequence
	 ...
	 * @return mediarepo_Core_Document[]|bool list of documents or false in case of an error
	 */
	function getDocuments($orderby="", $dir="asc", $limit=0, $offset=0) { /* {{{ */
		...
		if (!isset($this->_documents)) {
			...
			foreach ($resArr as $row) {
//				array_push($this->_documents, new mediarepo_Core_Document($row["id"], $row["name"], $row["comment"], $row["date"], $row["expires"], $row["owner"], $row["folder"], $row["inheritAccess"], $row["defaultAccess"], isset($row["lockUser"])?$row["lockUser"]:NULL, $row["keywords"], $row["sequence"]));
				array_push($this->_documents, $this->_repo->getDocument($row["id"]));
			}
		}
	...
	function countChildren($user, $limit=10000) { /* {{{ */
		...
		if($cfolders < $limit) {
			foreach ($resArr as $row) {
				$folder = $this->_repo->getFolder($row["id"]);
		...
		if($folderids) {
			...
			if($cdocs < $limit) {
				foreach ($resArr as $row) {
					$document = $this->_repo->getDocument($row["id"]);
	 ...
	 * @param mediarepo_Core_User $keywords keywords of new document
	 * @param mediarepo_Core_DocumentCategory[] $categories list of category objects
	 ...
	 * @param mediarepo_Core_Workflow $workflow
	...
	function addDocument($name, $comment, $expires, $owner, $keywords, $categories, $tmpFile, $orgFileName, $fileType, $mimeType, $sequence, $reviewers=array(), $approvers=array(),$reqversion=0,$version_comment="", $attributes=array(), $version_attributes=array(), $workflow=null) { /* {{{ */
		...
		$document = $this->_repo->getDocument($db->getInsertID('tblDocuments'));
...
//		else $res = $document->addContent($comment, $owner, $tmpFile, $orgFileName, $fileType, $mimeType, $reviewers, $approvers,$reqversion, $version_attributes, $workflow);
		...
		if($attributes) {
			foreach($attributes as $attrdefid=>$attribute) {
				...
				if($attribute)
					if(!$document->setAttributeValue($this->_repo->getAttributeDefinition($attrdefid), $attribute)) {
						$document->remove();
						$db->rollbackTransaction();
						return false;
					}
		...
		if(isset($this->_repo->callbacks['onPostAddDocument'])) {
			foreach($this->_repo->callbacks['onPostAddDocument'] as $callback) {
					/** @noinspection PhpStatementHasEmptyBodyInspection */
	...
	protected function removeFromDatabase() { /* {{{ */
		...
		if(isset($this->_repo->callbacks['onPreRemoveFromDatabaseFolder'])) {
			foreach($this->_repo->callbacks['onPreRemoveFromDatabaseFolder'] as $callback) {
		...
		if(isset($this->_repo->callbacks['onPostRemoveFromDatabaseFolder'])) {
			foreach($this->_repo->callbacks['onPostRemoveFromDatabaseFolder'] as $callback) {
				/** @noinspection PhpStatementHasEmptyBodyInspection */
	...
	function remove() { /* {{{ */
		...
		if ($this->_id == $this->_repo->rootFolderID || !isset($this->_parentID) || ($this->_parentID == null) || ($this->_parentID == "") || ($this->_parentID == 0)) {
			return false;
		}

		/* Check if 'onPreRemoveFolder' callback is set */
		if(isset($this->_repo->callbacks['onPreRemoveFolder'])) {
			foreach($this->_repo->callbacks['onPreRemoveFolder'] as $callback) {
				$ret = call_user_func($callback[0], $callback[1], $this);
				if(is_bool($ret))
					return $ret;
			}
		...
		/* Check if 'onPostRemoveFolder' callback is set */
		if(isset($this->_repo->callbacks['onPostRemoveFolder'])) {
			foreach($this->_repo->callbacks['onPostRemoveFolder'] as $callback) {
	 ...
	 * @return bool|mediarepo_Core_GroupAccess|mediarepo_Core_UserAccess
	 */
	 ...
	 * {@link getAccessMode}
	 *
	 * @param mediarepo_Core_Group $group group for which access shall be checked
	 * @return integer access mode
	 */
	 ...
	 * @return mediarepo_Core_User[]|mediarepo_Core_Group[]|bool array with a the elements 'users' and 'groups' which
	...
	function addNotify($userOrGroupID, $isUser) { /* {{{ */
		...
		/** @var mediarepo_Core_User|mediarepo_Core_Group $obj */
	 ...
	 * {@see mediarepo_Core_Folder::getReadAccessList()} instead.
	 */
