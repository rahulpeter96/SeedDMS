 ...
 * @version    Release: 1.1.12
 */
class mediarepo_Lucene_IndexedDocument extends Zend_Search_Lucene_Document {
	 ...
	 * @param mediarepo_Core_Document $document
	 * @param null $convcmd
	 * @param bool $nocontent
	 * @param int $timeout
	 */
	public function __construct($repo, $document, $convcmd=null, $nocontent=false, $timeout=5) { /* {{{ */
		$this->errormsg = '';
		$_convcmd = array(
		...
		if($version && !$nocontent) {
			$path = $repo->contentDir . $version->getPath();
			$content = '';
