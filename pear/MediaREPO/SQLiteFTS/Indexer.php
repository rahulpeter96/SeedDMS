 ...
 * @version    Release: 1.0.9
 */
class mediarepo_SQLiteFTS_Indexer {
	/**
	...
	static function open($indexerDir) { /* {{{ */
		if(file_exists($indexerDir.'/index.db')) {
			return new mediarepo_SQLiteFTS_Indexer($indexerDir);
		} else
	...
	static function create($indexerDir) { /* {{{ */
		...
		$index =  new mediarepo_SQLiteFTS_Indexer($indexerDir);
		/* Make sure the sequence of fields is identical to the field list
		 * in mediarepo_SQLiteFTS_Term
		 */
	 ...
	 * mediarepo_SQLiteFTS_IndexedDocument
	...
	public function find($query) { /* {{{ */
		...
		if($res) {
			foreach($res as $rec) {
				$hit = new mediarepo_SQLiteFTS_QueryHit($this);
				$hit->id = $rec['docid'];
				$hits[] = $hit;
			}
		}
	...
	public function getDocument($id) { /* {{{ */
		...
		if($res) {
			...
			$doc = new mediarepo_SQLiteFTS_Document();
	...
	public function terms() { /* {{{ */
		...
		if($res) {
			while($rec = $res->fetch(PDO::FETCH_ASSOC)) {
				$term = new mediarepo_SQLiteFTS_Term($rec['term'], $rec['col'], $rec['occurrences']);
				$terms[] = $term;
			}
		}
		return $terms;
