 ...
 * @version    Release: 1.2.9
 */
class mediarepo_Preview_PdfPreviewer extends mediarepo_Preview_Base {
	...
	protected function getFileName($object) { /* {{{ */
		...
		switch(get_class($object)) {
			case "mediarepo_Core_DocumentContent":
				$target = $dir.'p'.$object->getVersion();
				break;
			case "mediarepo_Core_DocumentFile":
	...
	public function createRawPreview($infile, $dir, $mimetype, $target='') { /* {{{ */
		...
		if(!is_dir($this->previewDir.'/'.$dir)) {
			if (!mediarepo_Core_File::makeDir($this->previewDir.'/'.$dir)) {
				return false;
			}
		}
	 ...
	 * {@link mediarepo_Preview::createRawPreview()}. The filename of the
	 * preview image is created by {@link mediarepo_Preview_Previewer::getFileName()}
	 *
	 * @param object $object instance of mediarepo_Core_DocumentContent
	 * or mediarepo_Core_DocumentFile
	 * @return boolean true on success, false on failure
	 */
	public function createPreview($object) { /* {{{ */
		...
		$document = $object->getDocument();
		$file = $document->_repo->contentDir.$object->getPath();
	 ...
	 *
	 * This function is a companion to {@link mediarepo_Preview_Previewer::createRawPreview()}.
	 *
	 * @param string $infile name of input file including full path
	 * @param string $dir directory relative to $this->previewDir
	 ...
	 * Check if a preview image already exists.
	 *
	 * This function is a companion to {@link mediarepo_Preview_Previewer::createPreview()}.
	 *
	 * @param object $object instance of mediarepo_Core_DocumentContent
	 * or mediarepo_Core_DocumentFile
	 * @return boolean true if preview exists, otherwise false
	...
	static function recurseRmdir($dir) {
		...
		foreach ($files as $file) {
			(is_dir("$dir/$file")) ? mediarepo_Preview_Previewer::recurseRmdir("$dir/$file") : unlink("$dir/$file");
		}
		return rmdir($dir);
	}
	 ...
	 *
	 * @param object $document instance of mediarepo_Core_Document
	 * @return boolean true if deletion succeded or false if file does not exist
	 */
	public function deleteDocumentPreviews($document) { /* {{{ */
		...
		if(file_exists($dir) && is_dir($dir)) {
			return mediarepo_Preview_Previewer::recurseRmdir($dir);
		} else {
			return false;
		}
