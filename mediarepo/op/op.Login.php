...
function _printMessage($heading, $message) { /* {{{ */
	global $repo, $theme;
	$view = UI::factory($theme, 'Login', array('repo'=>$repo));
...
if($settings->_enableGuestLogin && (int) $settings->_guestID) {
	$guestUser = $repo->getUser((int) $settings->_guestID);
...
if(!$user && isset($GLOBALS['mediarepo_HOOKS']['authentication'])) {
	foreach($GLOBALS['mediarepo_HOOKS']['authentication'] as $authObj) {
		if(!$user && method_exists($authObj, 'authenticate')) {
			$user = $authObj->authenticate($repo, $settings, $login, $pwd);
		}
	}
}
...
if (isset($_COOKIE["myrepo_session"])) {
	...
	if(!$resArr = $session->load($repo_session)) {
		...
		setcookie("myrepo_session", $repo_session, time()-3600, $settings->_httpRoot, null, null, !$settings->_enableLargeFileUpload); //delete cookie
	...
	if($settings->_cookieLifetime)
		...
		$lifetime = 0;
	setcookie("myrepo_session", $id, $lifetime, $settings->_httpRoot, null, null, !$settings->_enableLargeFileUpload);
}
