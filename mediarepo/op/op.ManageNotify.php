...
function add_folder_notify($folder,$userid,$recursefolder,$recursedoc) {
	...
	if ($recursedoc){
		...
		$documents = mediarepo_Core_REPO::filterAccess($documents, $repo->getUser($userid), M_READ);
	...
	if ($recursefolder){
		...
		$subFolders = mediarepo_Core_REPO::filterAccess($subFolders, $repo->getUser($userid), M_READ);
...
if ($_GET["type"]=="document"){
	...
	if(!$documentid || !($document = $repo->getDocument($documentid))) {
	...
	if(!$folderid || !($folder = $repo->getFolder($folderid))) {
