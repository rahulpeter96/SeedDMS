...
if ($action == "addgroup") {
	...
	if (is_object($repo->getGroupByName($name))) {
		UI::exitError(getMLText("admin_tools"),getMLText("group_exists"));
	}

	$newGroup = $repo->addGroup($name, $comment);
	if (!$newGroup) {
...
// Delete group -------------------------------------------------------------
	...
	$group = $repo->getGroup($_POST["groupid"]);
	if (!is_object($group)) {
		UI::exitError(getMLText("admin_tools"),getMLText("invalid_group_id"));
	}
...
// Add user to group --------------------------------------------------------
	...
	$newMember = $repo->getUser($_POST["userid"]);
...
// Remove user from group --------------------------------------------------
	...
	$oldMember = $repo->getUser($_POST["userid"]);
...
// toggle manager flag
	...
	$usertoedit = $repo->getUser($_POST["userid"]);
