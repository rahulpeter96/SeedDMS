...
if ($action == "addcategory") {
	...
	if (is_object($repo->getKeywordCategoryByName($name, $user->getID()))) {
		UI::exitError(getMLText("admin_tools"),getMLText("keyword_exists"));
	}
	$newCategory = $repo->addKeywordCategory($user->getID(), $name);
	if (!$newCategory) {
		UI::exitError(getMLText("admin_tools"),getMLText("error_occured"));
	}
...
// Delete keyword categorie ---------------------------------------------
	...
	$category = $repo->getKeywordCategory($categoryid);
