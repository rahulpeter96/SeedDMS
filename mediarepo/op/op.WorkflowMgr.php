...
if ($action == "addworkflow") {
	...
	$newWorkflow = $repo->addWorkflow($name, $state);
	if (!$newWorkflow) {
		UI::exitError(getMLText("admin_tools"),getMLText("error_occured"));
	}
...
// delete workflow --------------------------------------------------------
	...
	$workflowToRemove = $repo->getWorkflow($workflowid);
...
// modify workflow ---------------------------------------------------------
	...
	$workflowid=$_POST["workflowid"];
	$editedWorkflow = $repo->getWorkflow($workflowid);
