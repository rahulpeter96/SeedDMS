...
if ($action == "addworkflowstate") {
	...
	if (is_object($repo->getWorkflowStateByName($name))) {
		UI::exitError(getMLText("admin_tools"),getMLText("workflow_state_exists"));
	}

	$newWorkflowstate = $repo->addWorkflowState($name, $docstatus);
	if (!$newWorkflowstate) {
...
// delete user ------------------------------------------------------------
	...
	$workflowStateToRemove = $repo->getWorkflowState($workflowstateid);
...
// modify workflow ---------------------------------------------------------
	...
	$workflowstateid=$_POST["workflowstateid"];
	$editedWorkflowState = $repo->getWorkflowState($workflowstateid);
