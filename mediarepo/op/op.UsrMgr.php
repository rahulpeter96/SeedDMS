...
if ($action == "adduser") {
	...
	if (is_object($repo->getUserByLogin($login))) {
		UI::exitError(getMLText("admin_tools"),getMLText("user_exists"));
	}

	$newUser = $repo->addUser($login, md5($pwd), $name, $email, $settings->_language, $settings->_theme, $comment, $role, $isHidden, $isDisabled, $pwdexpiration, $quota, $homefolder);
	if ($newUser) {
	...
	if(isset($_POST["workflows"]) && $_POST["workflows"]) {
		...
		foreach($_POST["workflows"] as $workflowid)
			if($tmp = $repo->getWorkflow($workflowid))
				$workflows[] = $tmp;
		if($workflows)
...
// delete user ------------------------------------------------------------
	...
	$userToRemove = $repo->getUser($userid);
	if (!is_object($userToRemove)) {
		UI::exitError(getMLText("admin_tools"),getMLText("invalid_user_id"));
	}

	$userToAssign = $repo->getUser($_POST["assignTo"]);
...
// modify user ------------------------------------------------------------
	...
	$userid=$_POST["userid"];
	$editedUser = $repo->getUser($userid);
	...
	if(($role == mediarepo_Core_User::role_guest) && $clearpwd) {
		$editedUser->setPwd('');
	} else {
		if (isset($pwd) && ($pwd != "")) {
