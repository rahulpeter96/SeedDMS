...
elseif (isset($_GET["file"])) { /* {{{ */
	...
	header("Content-Length: " . filesize($repo->contentDir . $file->getPath() ));
...
elseif (isset($_GET["vfile"])) { /* {{{ */
	...
	header("Content-Type: text/plain");
	header("Content-Transfer-Encoding: binary");
	header("Content-Length: " . filesize($repo->contentDir.$document->getDir().$settings->_versioningFileName )."\"");
	$efilename = rawurlencode($settings->_versioningFileName);
	header("Content-Disposition: attachment; filename=\"". $efilename . "\"");
	header("Cache-Control: must-revalidate");
	
	sendFile($repo->contentDir . $document->getDir() .$settings->_versioningFileName);
...
elseif (isset($_GET["dumpname"])) { /* {{{ */
	...
	$filename = $repo->contentDir . $document->getDir().'r'.(int) $_GET['reviewlogid'];
...
elseif (isset($_GET["approvelogid"])) { /* {{{ */
	...
	$filename = $repo->contentDir . $document->getDir().'a'.(int) $_GET['approvelogid'];
	if (!file_exists($filename) ) {
		UI::exitError(getMLText("admin_tools"),getMLText("missing_file"));
	}
