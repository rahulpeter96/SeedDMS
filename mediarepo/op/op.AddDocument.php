...
$controller = Controller::factory($tmp[1], array('repo'=>$repo, 'user'=>$user));
...
if($settings->_quota > 0) {
	...
	if ($remain < 0) {
		UI::exitError(getMLText("folder_title", array("foldername" => htmlspecialchars($folder->getName()))),getMLText("quota_exceeded", array('bytes'=>mediarepo_Core_File::format_filesize(abs($remain)))));
	}
}
...
if($categories) {
	foreach($categories as $catid) {
		$cats[] = $repo->getDocumentCategory($catid);
	}
}
...
if($settings->_workflowMode == 'traditional' || $settings->_workflowMode == 'traditional_only_approval') {
	...
	if(!$workflows = $user->getMandatoryWorkflows()) {
		if(isset($_POST["workflow"]))
			$workflow = $repo->getWorkflow($_POST["workflow"]);
		else
			$workflow = null;
	} else {
...
if(!empty($_POST['notification_users'])) {
	foreach($_POST['notification_users'] as $notuserid) {
		$notuser = $repo->getUser($notuserid);
		if($notuser) {
			$notusers[] = $notuser;
		}
	}
...
if(!empty($_POST['notification_groups'])) {
	foreach($_POST['notification_groups'] as $notgroupid) {
		$notgroup = $repo->getGroup($notgroupid);
		if($notgroup) {
			$notgroups[] = $notgroup;
		}
	}
