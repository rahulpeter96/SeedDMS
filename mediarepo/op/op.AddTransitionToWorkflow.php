...
$state = $repo->getWorkflowState($_POST['state']);
if (!is_object($state)) {
	UI::exitError(getMLText("workflow_title"),getMLText("invalid_workflow_state_id"));
}
...
$nextstate = $repo->getWorkflowState($_POST['nextstate']);
...
$action = $repo->getWorkflowAction($_POST['action']);
if (!is_object($action)) {
	UI::exitError(getMLText("workflow_title"),getMLText("invalid_workflow_action_id"));
}
