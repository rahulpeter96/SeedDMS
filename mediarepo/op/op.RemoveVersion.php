	...
	if (!$document->removeContent($version)) {
		...
		if($settings->_enableFullSearch) {
			...
			if($index) {
				...
				$index->addDocument(new $indexconf['IndexedDocument']($repo, $document, isset($settings->_converters['fulltext']) ? $settings->_converters['fulltext'] : null, !($version->getFileSize() < $settings->_maxSizeForFullText)));
				$index->commit();
			}
		}
		...
		if ($notifier){
			...
			foreach ($emailUserList as $eID) {
				$eU = $version->_document->_repo->getUser($eID);
				$userrecipients[] = $eU;
			}
			$grouprecipients = array();
			foreach ($emailGroupList as $eID) {
				$eU = $version->_document->_repo->getGroup($eID);
				$grouprecipients[] = $eU;
			}
