...
if ($action == "addworkflowaction") {
	...
	if (is_object($repo->getWorkflowActionByName($name))) {
		UI::exitError(getMLText("admin_tools"),getMLText("workflow_action_exists"));
	}

	$newWorkflowaction = $repo->addWorkflowAction($name);
...
// delete user ------------------------------------------------------------
	...
	$workflowActionToRemove = $repo->getWorkflowAction($workflowactionid);
...
// modify workflow ---------------------------------------------------------
	...
	$workflowactionid=$_POST["workflowactionid"];
	$editedWorkflowAction = $repo->getWorkflowAction($workflowactionid);
