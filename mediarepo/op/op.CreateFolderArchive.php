...
function createFolderTar($folder,$ark, $human_readable, $repo) { /* {{{ */
	$documents=$folder->getDocuments();
	foreach ($documents as $document){

		if (file_exists($repo->contentDir.$document->getDir())){

			if ($human_readable){
				// create an archive containing the files with original names and REPO path
				// thanks to Doudoux
				$latestContent = $document->getLatestContent();
				if (is_object($latestContent)) {
					TarAddHeader(
						$ark,
						$repo->contentDir.$latestContent->getPath(),
						getFolderPathPlainAST($folder)."/".$document->getID()."_".$latestContent->getOriginalFileName());

					TarWriteContents($ark, $repo->contentDir.$latestContent->getPath());
				}
			} else {

				// create a server backup archive
				$handle = opendir($repo->contentDir.$document->getDir());
				while ($entry = readdir($handle) ) {
					if (!is_dir($repo->contentDir.$document->getDir().$entry)){

						TarAddHeader($ark,$repo->contentDir.$document->getDir().$entry,$document->getDir().$entry);
						TarWriteContents($ark,$repo->contentDir.$document->getDir().$entry);
					}
				}
				closedir($handle);
			}
	...
	foreach ($subFolders as $folder)
		if (!createFolderTar($folder,$ark,$human_readable,$repo))
			return false;
...
if (mediarepo_Core_File::gzcompressfile($ark_name,9)) unlink($ark_name);
