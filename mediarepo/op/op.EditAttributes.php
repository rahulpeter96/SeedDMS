	...
	foreach($attributes as $attrdefid=>$attribute) {
		...
		if($attribute) {
			...
			if(!isset($oldattributes[$attrdefid]) || $attribute != $oldattributes[$attrdefid]->getValue()) {
				if(!$version->setAttributeValue($repo->getAttributeDefinition($attrdefid), $attribute)) {
			...
			if(!$version->removeAttribute($repo->getAttributeDefinition($attrdefid)))
	...
	foreach($newattributes as $attrdefid=>$attribute) {
		if(!isset($oldattributes[$attrdefid]) && $attribute) {
			if($notifier) {
				...
				$params['name'] = $document->getName();
				$params['version'] = '';
				$params['attribute_name'] = $repo->getAttributeDefinition($attrdefid)->getName();
