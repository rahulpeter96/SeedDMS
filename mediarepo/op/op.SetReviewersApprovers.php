	...
	foreach ($reviewIndex["i"] as $rx=>$rv) {
		if ($rv["status"] == 0) {
			...
			if (!isset($docAccess["users"][$accessIndex["i"][$rx]])) {
				 ...
				 */
				$res = $content->delIndReviewer($repo->getUser($reviewStatus[$rv["idx"]]["required"]), $user, getMLText("removed_reviewer"));
			}
			else {
	...
	foreach ($reviewIndex["g"] as $rx=>$rv) {
		if ($rv["status"] == 0) {
			...
			if (!isset($docAccess["groups"][$accessIndex["g"][$rx]])) {
				...
				$res = $db->getResult($queryStr);
				 */
				$res = $content->delGrpReviewer($repo->getGroup($reviewStatus[$rv["idx"]]["required"]), $user, getMLText("removed_reviewer"));
			}
			else {
	...
	foreach ($approvalIndex["i"] as $rx=>$rv) {
		if ($rv["status"] == 0) {
			...
			if (!isset($docAccess["users"][$accessIndex["i"][$rx]])) {
				...
				$res = $db->getResult($queryStr);
				 */
				$res = $content->delIndApprover($repo->getUser($approvalStatus[$rv["idx"]]["required"]), $user, getMLText("removed_approver"));
			}
			else {
	...
	foreach ($approvalIndex["g"] as $rx=>$rv) {
		if ($rv["status"] == 0) {
			...
			if (!isset($docAccess["groups"][$accessIndex["g"][$rx]])) {
				...
				$res = $db->getResult($queryStr);
*/
				$res = $content->delGrpApprover($repo->getGroup($approvalStatus[$rv["idx"]]["required"]), $user, getMLText("removed_approver"));
			}
			else {
