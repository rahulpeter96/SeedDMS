...
if (isset($_COOKIE["myrepo_session"])) {
	...
	if($settings->_enableEmail) {
		$notifier->addService(new mediarepo_EmailNotify($repo));
	}
...
switch($command) {
	...
	case 'searchdocument': /* {{{ */
		if($user) {
			...
			$hits = $repo->search($query, $limit=0, $offset=0, $logicalmode='AND', $searchin=array(), $startFolder=null, $owner=null, $status = array(), $creationstartdate=array(), $creationenddate=array(), $modificationstartdate=array(), $modificationenddate=array(), $categories=array(), $attributes=array(), $mode=0x1, $expirationstartdate=array(), $expirationenddate=array());
			if($hits) {
				$result = array();
	...
	case 'searchfolder': /* {{{ */
		if($user) {
			...
			$hits = $repo->search($query, $limit=0, $offset=0, $logicalmode='AND', $searchin=array(), $startFolder=null, $owner=null, $status = array(), $creationstartdate=array(), $creationenddate=array(), $modificationstartdate=array(), $modificationenddate=array(), $categories=array(), $attributes=array(), $mode=0x2, $expirationstartdate=array(), $expirationenddate=array());
			if($hits) {
				$result = array();
	...
	case 'subtree': /* {{{ */
		if($user) {
			...
			$folder = $repo->getFolder($nodeid);
			if (!is_object($folder)) return '';
			
			$subfolders = $folder->getSubFolders($orderby);
			$subfolders = mediarepo_Core_REPO::filterAccess($subfolders, $user, M_READ);
			$tree = array();
			...
			if($showdocs) {
				...
				$documents = mediarepo_Core_REPO::filterAccess($documents, $user, M_READ);
	...
	case 'addtoclipboard': /* {{{ */
		if($user) {
			if (isset($_GET["id"]) && is_numeric($_GET["id"]) && isset($_GET['type'])) {
				switch($_GET['type']) {
					case "folder":
						$session->addToClipboard($repo->getFolder($_GET['id']));
						break;
					case "document":
						$session->addToClipboard($repo->getDocument($_GET['id']));
						break;
				}
				header('Content-Type: application/json');
	...
	case 'removefromclipboard': /* {{{ */
		if($user) {
			if (isset($_GET["id"]) && is_numeric($_GET["id"]) && isset($_GET['type'])) {
				switch($_GET['type']) {
					case "folder":
						$session->removeFromClipboard($repo->getFolder($_GET['id']));
						break;
					case "document":
						$session->removeFromClipboard($repo->getDocument($_GET['id']));
						break;
				}
				header('Content-Type: application/json');
	...
	case 'testmail': /* {{{ */
		if($user && $user->isAdmin()) {
			if($user->getEmail()) {
				$emailobj = new mediarepo_EmailNotify($repo, $settings->_smtpSendFrom, $settings->_smtpServer, $settings->_smtpPort, $settings->_smtpUser, $settings->_smtpPassword);
				$params = array();
	...
	case 'movefolder': /* {{{ */
		if($user) {
			if(!checkFormKey('movefolder', 'GET')) {
				...
				$mfolder = $repo->getFolder($_REQUEST['folderid']);
				if($mfolder) {
					if ($mfolder->getAccessMode($user) >= M_READWRITE) {
						if($folder = $repo->getFolder($_REQUEST['targetfolderid'])) {
	...
	case 'movedocument': /* {{{ */
		if($user) {
			if(!checkFormKey('movedocument', 'GET')) {
				...
				$mdocument = $repo->getDocument($_REQUEST['docid']);
				if($mdocument) {
	...
	case 'deletefolder': /* {{{ */
		if($user) {
			if(!checkFormKey('removefolder', 'GET')) {
				...
				$folder = $repo->getFolder($_REQUEST['id']);
				if($folder) {
					if ($folder->getAccessMode($user) >= M_READWRITE) {
	...
	case 'deletedocument': /* {{{ */
		if($user) {
			if(!checkFormKey('removedocument', 'GET')) {
				...
				$document = $repo->getDocument($_REQUEST['id']);
				if($document) {
		...
		if($user) {
			if(checkFormKey('adddocument')) {
				...
				if($settings->_quota > 0) {
					...
					if ($remain < 0) {
						header('Content-Type: application/json');
						echo json_encode(array('success'=>false, 'message'=>getMLText("quota_exceeded", array('bytes'=>mediarepo_Core_File::format_filesize(abs($remain))))));
						exit;
					}
				}
				...
				$controller = Controller::factory('AddDocument', array('repo'=>$repo, 'user'=>$user));
						...
						if($settings->_enableNotificationAppRev) {
							...
							if($reviewers['i'] || $reviewers['g']) {
								...
								foreach($reviewers['i'] as $reviewerid) {
									$notifier->toIndividual($user, $repo->getUser($reviewerid), $subject, $message, $params);
								}
								foreach($reviewers['g'] as $reviewergrpid) {
									$notifier->toGroup($user, $repo->getGroup($reviewergrpid), $subject, $message, $params);
								}
							}
							...
							elseif($approvers['i'] || $approvers['g']) {
								...
								foreach($approvers['i'] as $approverid) {
									$notifier->toIndividual($user, $repo->getUser($approverid), $subject, $message, $params);
								}
								foreach($approvers['g'] as $approvergrpid) {
									$notifier->toGroup($user, $repo->getGroup($approvergrpid), $subject, $message, $params);
								}
							}
						}
