...
if($settings->_quota > 0) {
	...
	if ($remain < 0) {
		UI::exitError(getMLText("document_title", array("documentname" => htmlspecialchars($document->getName()))),getMLText("quota_exceeded", array('bytes'=>mediarepo_Core_File::format_filesize(abs($remain)))));
	}
}
...
/* Check if the uploaded file is identical to last version */
	...
	if($lc->getChecksum() == mediarepo_Core_File::checksum($userfiletmp)) {
