<?php
if(isset($_SERVER['mediarepo_HOME'])) {
	require_once($_SERVER['mediarepo_HOME']."/inc/inc.ClassSettings.php");
} else {
...
function usage() { /* {{{ */
	echo "Usage:\n";
	echo "  mediarepo-xmlimport [-h] [-v] [--config <file>]\n";
	echo "\n";
	echo "Description:\n";
	echo "  This program imports an xml dump into the repo.\n";
	echo "\n";
	echo "Options:\n";
	...
	foreach($reviews as $i=>$review) {
		...
		if($review['attributes']['type'] == 1) {
			if(isset($objmap['groups'][(int) $review['attributes']['required']]))
				$newreview['required'] = $repo->getGroup($objmap['groups'][(int) $review['attributes']['required']]);
			else
			...
			if(isset($objmap['users'][(int) $review['attributes']['required']]))
				$newreview['required'] = $repo->getUser($objmap['users'][(int) $review['attributes']['required']]);
			else
		...
		foreach($review['logs'] as $j=>$log) {
			if(!array_key_exists($log['attributes']['user'], $objmap['users'])) {
				...
				$newlog = array();
				$newlog['user'] = $repo->getUser($objmap['users'][$log['attributes']['user']]);
...
function getWorkflowLog($workflowlogs) { /* {{{ */
	...
	foreach($workflowlogs as $i=>$log) {
		if(!array_key_exists($log['attributes']['user'], $objmap['users'])) {
			...
			$log['attributes']['user'] = $repo->getUser($objmap['users'][$log['attributes']['user']]);
			if(!array_key_exists($log['attributes']['workflow'], $objmap['workflows'])) {
				unset($initversion['workflowlogs'][$i]);
				$logger->warning("Workflow for workflow log cannot be mapped");
			} else {
				$log['attributes']['workflow'] = $repo->getWorkflow($objmap['workflows'][$log['attributes']['workflow']]);
				if(!array_key_exists($log['attributes']['transition'], $objmap['workflowtransitions'])) {
					...
					$log['attributes']['transition'] = $repo->getWorkflowTransition($objmap['workflowtransitions'][$log['attributes']['transition']]);
...
function insert_user($user) { /* {{{ */
	global $logger, $repo, $debug, $sections, $defaultUser, $objmap;

	if($debug) print_r($user);

	if ($newUser = $repo->getUserByLogin($user['attributes']['login'])) {
		...
		if(in_array('users', $sections)) {
			...
			if(!$newUser) {
				...
				$logger->debug($repo->getDB()->getErrorMsg());
				return false;
			} else {
...
function insert_group($group) { /* {{{ */
	global $logger, $repo, $debug, $objmap, $sections, $users;

	if($debug) print_r($group);

	if ($newGroup = $repo->getGroupByName($group['attributes']['name'])) {
		$logger->warning("Group already exists");
	} else {
		if(in_array('groups', $sections)) {
			$newGroup = $repo->addGroup($group['attributes']['name'], $group['attributes']['comment']);
			if($newGroup) {
				...
				foreach($group['users'] as $guser) {
					...
					if(isset($users[$guser])) {
						$user = $users[$guser];
						if($newMember = $repo->getUserByLogin($user['attributes']['login'])) {
...
function insert_attributedefinition($attrdef) { /* {{{ */
	global $logger, $repo, $debug, $objmap, $sections;

	if($debug)
		print_r($attrdef);
	if($newAttrdef = $repo->getAttributeDefinitionByName($attrdef['attributes']['name'])) {
		$logger->warning("Attribute definition already exists");
	} else {
		if(in_array('attributedefinitions', $sections)) {
			$objtype = ($attrdef['objecttype'] == 'folder' ? mediarepo_Core_AttributeDefinition::objtype_folder : ($attrdef['objecttype'] == 'document' ? mediarepo_Core_AttributeDefinition::objtype_document : ($attrdef['objecttype'] == 'documentcontent' ? mediarepo_Core_AttributeDefinition::objtype_documentcontent : 0)));
			if(!$newAttrdef = $repo->addAttributeDefinition($attrdef['attributes']['name'], $objtype, $attrdef['attributes']['type'], $attrdef['attributes']['multiple'], $attrdef['attributes']['minvalues'], $attrdef['attributes']['maxvalues'], $attrdef['attributes']['valueset'], $attrdef['attributes']['regex'])) {
...
function insert_documentcategory($documentcat) { /* {{{ */
	...
	if($newCategory = $repo->getDocumentCategoryByName($documentcat['attributes']['name'])) {
		$logger->warning("Document category already exists");
	} else {
		if(in_array('documentcategories', $sections)) {
			if(!$newCategory = $repo->addDocumentCategory($documentcat['attributes']['name'])) {
...
function insert_keywordcategory($keywordcat) { /* {{{ */
	...
	if($newCategory = $repo->getKeywordCategoryByName($keywordcat['attributes']['name'], $owner)) {
		$logger->warning("Keyword category already exists");
	} else {
		if(in_array('keywordcategories', $sections)) {
			if(!$newCategory = $repo->addKeywordCategory($owner, $keywordcat['attributes']['name'])) {
...
function insert_workflow($workflow) { /* {{{ */
	...
	if($debug)
		print_r($workflow);
	if($newWorkflow = $repo->getWorkflowByName($workflow['attributes']['name'])) {
		$logger->warning("Workflow already exists");
	} else {
		if(in_array('workflows', $sections)) {
			if(!$initstate = $repo->getWorkflowState($objmap['workflowstates'][(int)$workflow['attributes']['initstate']])) {
				$logger->err("Could not add workflow because initial state is missing");
				return false;
			}
			if(!$newWorkflow = $repo->addWorkflow($workflow['attributes']['name'], $initstate)) {
			...
			if($workflow['transitions']) {
				foreach($workflow['transitions'] as $transition) {
					if(!$state = $repo->getWorkflowState($objmap['workflowstates'][(int) $transition['attributes']['startstate']])) {
						$logger->err("Could not add workflow because start state of transition is missing");
						$logger->debug($repo->getDB()->getErrorMsg());
						return false;
					}
					if(!$nextstate = $repo->getWorkflowState($objmap['workflowstates'][(int) $transition['attributes']['nextstate']])) {
						$logger->err("Could not add workflow because end state of transition is missing");
						$logger->debug($repo->getDB()->getErrorMsg());
						return false;
					}
					if(!$action = $repo->getWorkflowAction($objmap['workflowactions'][(int) $transition['attributes']['action']])) {
						$logger->err("Could not add workflow because end state of transition is missing");
						$logger->debug($repo->getDB()->getErrorMsg());
						return false;
					}
					...
					if($transition['users']) {
						foreach($transition['users'] as $tuserid) {
							if(!$tusers[] = $repo->getUser($objmap['users'][(int) $tuserid])) {
					...
					if($transition['groups']) {
						foreach($transition['groups'] as $tgroupid) {
							if(!$tgroups[] = $repo->getGroup($objmap['groups'][(int) $tgroupid])) {
...
function insert_workflowstate($workflowstate) { /* {{{ */
	...
	if($newWorkflowstate = $repo->getWorkflowStateByName($workflowstate['attributes']['name'])) {
		$logger->warning("Workflow state already exists");
	} else {
		if(in_array('workflows', $sections)) {
			if(!$newWorkflowstate = $repo->addWorkflowState($workflowstate['attributes']['name'], isset($workflowstate['attributes']['documentstate']) ? $workflowstate['attributes']['documentstate'] : 0)) {
...
function insert_workflowaction($workflowaction) { /* {{{ */
	...
	if($newWorkflowaction = $repo->getWorkflowActionByName($workflowaction['attributes']['name'])) {
		$logger->warning("Workflow action already exists");
	} else {
		if(in_array('workflows', $sections)) {
			if(!$newWorkflowaction = $repo->addWorkflowAction($workflowaction['attributes']['name'])) {
...
function insert_document($document) { /* {{{ */
	global $logger, $repo, $debug, $defaultUser, $objmap, $sections, $rootfolder, $contentdir;
	...
	if(!array_key_exists((int) $document['attributes']['owner'], $objmap['users'])) {
		...
		$owner = $repo->getUser($objmap['users'][(int) $document['attributes']['owner']]);
	}
	...
	if(isset($document['categories'])) {
		foreach($document['categories'] as $catid) {
			if(array_key_exists((int) $catid, $objmap['documentcategories'])) {
				$categories[$objmap['documentcategories'][$catid]] = $repo->getDocumentCategory($objmap['documentcategories'][$catid]);
			} else {
	...
	if(isset($document['folder']) && $document['folder']) {
		if(array_key_exists($document['folder'], $objmap['folders'])) {
			$folder = $repo->getFolder($objmap['folders'][$document['folder']]);
		} else {
	...
	if(in_array('documents', $sections)) {
		...
		if(isset($initversion['workflow']) && $initversion['workflow']) {
			if(array_key_exists((int) $initversion['workflow']['id'], $objmap['workflows'])) {
				$workflow = $repo->getWorkflow($objmap['workflows'][(int) $initversion['workflow']['id']]);
				if(!$workflow) {
			...
			if(array_key_exists((int) $initversion['workflow']['state'], $objmap['workflowstates'])) {
				$workflowstate = $repo->getWorkflowState($objmap['workflowstates'][(int) $initversion['workflow']['state']]);
				if(!$workflowstate) {
		...
		if(isset($document['attributes']['lockedby'])) {
			if(!array_key_exists($document['attributes']['lockedby'], $objmap['users'])) {
				...
				if($lockuser = $repo->getUser($objmap['users'][$document['attributes']['lockedby']])) {
...
//			print_r($initversion['reviews']);
		...
		foreach($document['versions'] as $version) {
			...
			$owner = $repo->getUser($objmap['users'][(int) $version['attributes']['owner']]);
			...
			if(isset($version['workflow']) && $version['workflow']) {
				if(array_key_exists((int) $version['workflow']['id'], $objmap['workflows'])) {
					$workflow = $repo->getWorkflow($objmap['workflows'][(int) $version['workflow']['id']]);
					if(!$workflow) {
				...
				if(array_key_exists((int) $version['workflow']['state'], $objmap['workflowstates'])) {
					$workflowstate = $repo->getWorkflowState($objmap['workflowstates'][(int) $version['workflow']['state']]);
					if(!$workflowstate) {
		...
		if(isset($document['files']) && $document['files']) {
			foreach($document['files'] as $file) {
				if(!array_key_exists($file['attributes']['owner'], $objmap['users'])) {
					...
					$owner = $repo->getUser($objmap['users'][$file['attributes']['owner']]);
				}
...
function insert_folder($folder) { /* {{{ */
	global $logger, $repo, $debug, $objmap, $defaultUser, $sections, $rootfolder;
	...
	if(in_array('folders', $sections)) {
		if(!array_key_exists($folder['attributes']['owner'], $objmap['users'])) {
			...
			$owner = $repo->getUser($objmap['users'][(int) $folder['attributes']['owner']]);
		}
		...
		if(isset($folder['folder']) && $folder['folder']) {
			if(array_key_exists($folder['folder'], $objmap['folders'])) {
				$parent = $repo->getFolder($objmap['folders'][$folder['folder']]);
			} else {
...
function resolve_links() { /* {{{ */
	global $logger, $repo, $debug, $defaultUser, $links, $objmap;
	...
	foreach($links as $documentid=>$doclinks) {
		if(array_key_exists($documentid, $objmap['documents'])) {
			if($doc = $repo->getDocument($objmap['documents'][$documentid])) {
				foreach($doclinks as $doclink) {
							if(array_key_exists($doclink['attributes']['target'], $objmap['documents'])) {
								if($target = $repo->getDocument($objmap['documents'][$doclink['attributes']['target']])) {
									if(!array_key_exists($doclink['attributes']['owner'], $objmap['users'])) {
										...
										$owner = $repo->getUser($objmap['users'][$doclink['attributes']['owner']]);
									}
...
function set_mandatory() { /* {{{ */
	global $logger, $repo, $users, $objmap;
		...
		if ($newUser = $repo->getUserByLogin($user['attributes']['login'])) {
			if($user['individual']['reviewers']) {
				foreach($user['individual']['reviewers'] as $u) {
					if($uobj = $repo->getUser($objmap['users'][$u])) {
			...
			if($user['group']['reviewers']) {
				foreach($user['group']['reviewers'] as $u) {
					if($uobj = $repo->getGroup($objmap['groups'][$u])) {
...
$rootfolder = $repo->getFolder($folderid);
if(!$rootfolder) {
	exit(1);
}

if($defaultuserid) {
	if(!$defaultUser = $repo->getUser($defaultuserid)) {
