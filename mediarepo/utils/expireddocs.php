...
if(isset($_SERVER['mediarepo_HOME'])) {
	ini_set('include_path', $_SERVER['mediarepo_HOME']. PATH_SEPARATOR .ini_get('include_path'));
}
require_once("inc/inc.ClassSettings.php");

function usage() { /* {{{ */
	echo "Usage:\n";
	echo "  mediarepo-expireddocs [--config <file>] [-u <user>] [-h] [-v] [-t] [-q] [-o] [-f <email>] [-u <user>] [-w] [-b <base>] [-c] -d <days> -D <days>\n";
	echo "\n";
	echo "Description:\n";
...
foreach($users as $user) {
	...
	$body = "";
	$bodyhtml = "<html>\n<head>\n<meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" />\n<title>MediaREPO: ".getMLText('daylyDigestMail', array(), "", $user->getLanguage())."</title>\n<base href=\"".$baseurl.$settings->_httpRoot."\" />\n</head>\n<body>\n";
	...
	if (count($resArr)>0) {
		...
		foreach ($resArr as $res) {
			if($doc = $repo->getDocument((int) $res['id'])) {
				$catnames = array();
	...
	if($sendmail) {
		if($user->getEmail()) {
			...
			if(!$dryrun) {
				...
				$hdrs = $mime->headers(array('From' => $mailfrom, 'Subject' => 'MediaREPO: '.getMLText('daylyDigestMail', array(), "", $user->getLanguage()), 'Content-Type' => 'text/plain; charset=UTF-8'));
