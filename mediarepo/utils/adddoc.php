...
if(isset($_SERVER['mediarepo_HOME'])) {
	ini_set('include_path', $_SERVER['mediarepo_HOME'].'/utils'. PATH_SEPARATOR .ini_get('include_path'));
} else {
...
function usage() { /* {{{ */
	echo "Usage:\n";
	echo "  mediarepo-adddoc [--config <file>] [-c <comment>] [-k <keywords>] [-s <number>] [-n <name>] [-V <version>] [-s <sequence>] [-t <mimetype>] [-a <attribute=value>] [-h] [-v] -F <folder id> -f <filename>\n";
	echo "\n";
	echo "Description:\n";
	echo "  This program uploads a file into a folder of MediaREPO.\n";
	echo "\n";
	echo "Options:\n";
...
if(isset($options['config'])) {
	define('mediarepo_CONFIG_FILE', $options['config']);
}
...
if(isset($options['K'])) {
	...
	foreach($categorynames as $categoryname) {
		$cat = $repo->getDocumentCategoryByName($categoryname);
		if($cat) {
			$categories[] = $cat;
		} else {
...
if (isset($options['a'])) {
	...
	foreach ($docattr as $thisAttribute) {
		...
		$attrdef = $repo->getAttributeDefinitionByName($attrKey);
		if (!$attrdef) {
