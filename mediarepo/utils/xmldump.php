...
function usage() { /* {{{ */
	echo "Usage:\n";
	echo "  mediarepo-xmldump [-h] [-v] [--config <file>]\n";
	echo "\n";
	echo "Description:\n";
	echo "  This program creates an xml dump of the whole or parts of the repo.\n";
	echo "\n";
	echo "Options:\n";
...
function dumplog($version, $type, $logs, $indent) { /* {{{ */
	global $repo, $contentdir, $maxsize;
	...
	foreach($logs as $a) {
		...
		if(!empty($a['file'])) {
			$filename = $repo->contentDir . $document->getDir().'r'.(int) $a[$type2.'LogID'];
...
function tree($folder, $parent=null, $indent='', $skipcurrent=false) { /* {{{ */
	global $sections, $statistic, $index, $repo, $maxsize, $contentdir;
		...
		foreach($documents as $document) {
			...
			if($versions) {
				...
				foreach($versions as $version) {
					...
					if(file_exists($repo->contentDir . $version->getPath())) {
						echo $indent."   <data length=\"".filesize($repo->contentDir . $version->getPath())."\"";
						if(filesize($repo->contentDir . $version->getPath()) < $maxsize) {
							echo ">\n";
							echo chunk_split(base64_encode(file_get_contents($repo->contentDir . $version->getPath())), 76, "\n");
							echo $indent."   </data>\n";
						} else {
							echo " fileref=\"".$document->getID()."-".$version->getVersion().$version->getFileType()."\" />\n";
							if($contentdir) {
								copy($repo->contentDir . $version->getPath(), $contentdir.$document->getID()."-".$version->getVersion().$version->getFileType());
							} else {
								echo "Warning: version content (size=".filesize($repo->contentDir . $version->getPath()).") will be missing from output\n";
							}
						}
					} else {
						echo $indent."   <!-- ".$repo->contentDir . $version->getPath()." not found -->\n";
			...
			if($files) {
				...
				foreach($files as $file) {
					...
					if(file_exists($repo->contentDir . $file->getPath())) {
						echo $indent."   <data length=\"".filesize($repo->contentDir . $file->getPath())."\"";
						if(filesize($repo->contentDir . $file->getPath()) < $maxsize) {
							echo ">\n";
							echo chunk_split(base64_encode(file_get_contents($repo->contentDir . $file->getPath())), 76, "\n");
							echo $indent."   </data>\n";
						} else {
							echo " fileref=\"".$document->getID()."-A-".$file->getID().$file->getFileType()."\" />\n";
							if($contentdir) {
								copy($repo->contentDir . $file->getPath(), $contentdir.$document->getID()."-A-".$file->getID().$file->getFileType());
							} else {
								echo "Warning: file content (size=".filesize($repo->contentDir . $file->getPath()).") will be missing from output\n";
							}
						}
					} else {
						echo $indent."   <!-- ".$repo->contentDir . $version->getID()." not found -->\n";
					}
					echo $indent."  </file>\n";
				}
				echo $indent." </files>\n";
...
echo "<repo dbversion=\"".implode('.', array_slice($repo->getDBVersion(), 1, 3))."\" date=\"".date('Y-m-d H:i:s')."\">\n";
...
$categories = $repo->getAllKeywordCategories();
if($categories) {
...
$categories = $repo->getDocumentCategories();
if($categories) {
...
$workflowstates = $repo->getAllWorkflowStates();
if($workflowstates) {
	echo "<workflowstates>\n";
...
$workflowactions = $repo->getAllWorkflowActions();
if($workflowactions) {
...
$workflows = $repo->getAllWorkflows();
if($workflows) {
	echo "<workflows>\n";
