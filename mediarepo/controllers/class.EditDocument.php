 ...
 * @version    Release: @package_version@
 */
class mediarepo_Controller_EditDocument extends mediarepo_Controller_Common {
		...
		if($result === null) {
			...
			if($categories) {
				...
				foreach($categories as $catid) {
					if($cat = $repo->getDocumentCategory($catid)) {
						$categoriesarr[] = $cat;
					}
				...
				foreach($attributes as $attrdefid=>$attribute) {
					...
					if($attribute) {
						...
						if(!isset($oldattributes[$attrdefid]) || $attribute != $oldattributes[$attrdefid]->getValue()) {
							if(!$document->setAttributeValue($repo->getAttributeDefinition($attrdefid), $attribute))
								return false;
						}
					} elseif($attrdef->getMinValues() > 0) {
						$this->errormsg = getMLText("attr_min_values", array("attrname"=>$attrdef->getName()));
					} elseif(isset($oldattributes[$attrdefid])) {
						if(!$document->removeAttribute($repo->getAttributeDefinition($attrdefid)))
							return false;
					}
				}
			}
