 ...
 * @version    Release: @package_version@
 */
class mediarepo_Controller_UpdateDocument extends mediarepo_Controller_Common {
	...
	public function run() { /* {{{ */
		...
		if($result === null) {
			$filesize = mediarepo_Core_File::fileSize($userfiletmp);
			...
			if($index) {
				...
				$idoc = new $indexconf['IndexedDocument']($repo, $document, isset($settings->_converters['fulltext']) ? $settings->_converters['fulltext'] : null, !($filesize < $settings->_maxSizeForFullText));
