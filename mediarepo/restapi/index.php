...
if(USE_PHP_SESSION) {
    ...
    if(isset($_SESSION['userid']))
        $userobj = $repo->getUser($_SESSION['userid']);
    elseif($settings->_enableGuestLogin)
        $userobj = $repo->getUser($settings->_guestID);
    else
        exit;
    $repo->setUser($userobj);
} else {
    ...
    if (isset($_COOKIE["myrepo_session"])) {
        ...
        if(!$resArr = $session->load($repo_session)) {
            /* Delete Cookie */
            setcookie("myrepo_session", $repo_session, time()-3600, $settings->_httpRoot);
        ...
        /* Load user data */
        $userobj = $repo->getUser($resArr["userID"]);
        ...
        if($userobj->isAdmin()) {
            if($resArr["su"]) {
                $userobj = $repo->getUser($resArr["su"]);
            }
        }
        $repo->setUser($userobj);
    }
...
function __getUserData($u) { /* {{{ */
    $data = array(
        ...
        'theme' => $u->getTheme(),
        'role' => $u->getRole() == mediarepo_Core_User::role_admin ? 'admin' : ($u->getRole() == mediarepo_Core_User::role_guest ? 'guest' : 'user'),
...
function doLogin() { /* {{{ */
    global $app, $repo, $userobj, $session, $settings;
...
//    $userobj = $repo->getUserByLogin($username);
    ...
    if (!$userobj && isset($settings->_ldapHost) && strlen($settings->_ldapHost)>0) {
        ...
        $authobj = new mediarepo_LdapAuthentication($repo, $settings);
        $userobj = $authobj->authenticate($username, $password);
    } /* }}} */

    /* Authenticate against MediaREPO database {{{ */
    if(!$userobj) {
        require_once("../inc/inc.ClassDbAuthentication.php");
        $authobj = new mediarepo_DbAuthentication($repo, $settings);
        $userobj = $authobj->authenticate($username, $password);
    } /* }}} */
    ...
    if(!$userobj) {
        if(USE_PHP_SESSION) {
            ...
            setcookie("myrepo_session", $session->getId(), time()-3600, $settings->_httpRoot);
        }
        ...
        if(USE_PHP_SESSION) {
            ...
            if($settings->_cookieLifetime)
                ...
                $lifetime = 0;
            setcookie("myrepo_session", $id, $lifetime, $settings->_httpRoot);
            $repo->setUser($userobj);
        }
        $app->response()->header('Content-Type', 'application/json');
...
function getLockedDocuments() { /* {{{ */
    ...
    if(false !== ($documents = $repo->getDocumentsLockedByUser($userobj))) {
        $documents = mediarepo_Core_REPO::filterAccess($documents, $userobj, M_READ);
        $recs = array();
...
function getFolder($id = null) { /* {{{ */
    global $app, $repo, $userobj, $settings;
    ...
    if ($id === null)
        ...
        $folder = $repo->getFolder($id);
    else {
        $parentid = $app->request()->get('parentid');
        $folder = $repo->getFolderByName($id, $parentid);
    }
    if($folder) {
    ...
    if($id == 0) {
        ...
        if($folder) {
            if($folder->getAccessMode($userobj) >= M_READ) {
                ...
                $subfolders = mediarepo_Core_REPO::filterAccess($subfolders, $userobj, M_READ);
            ...
            if($name = $app->request()->post('name')) {
                ...
                if($attributes) {
                    foreach($attributes as $attrname=>$attrvalue) {
                        $attrdef = $repo->getAttributeDefinitionByName($attrname);
                        if($attrdef) {
...
function moveFolder($id, $folderid) { /* {{{ */
    ...
    $mfolder = $repo->getFolder($id);
    if($mfolder) {
        if ($mfolder->getAccessMode($userobj) >= M_READ) {
            if($folder = $repo->getFolder($folderid)) {
...
function uploadDocumentFile($documentId) { /* {{{ */
    ...
    $document = $repo->getDocument($documentId);
    if($document) {
    ...
    if($document) {
        if ($document->getAccessMode($userobj) >= M_READ) {
            ...
            if($lc) {
              ...
              $app->response()->header("Content-Length", filesize($repo->contentDir . $lc->getPath()));
              ...
              sendFile($repo->contentDir . $lc->getPath());
            } else {
              $app->response()->status(403);
              $app->response()->header('Content-Type', 'application/json');
              echo json_encode(array('success'=>false, 'message'=>'No access', 'data'=>''));
...
function getDocumentFile($id, $fileid) { /* {{{ */
    ...
    if($document) {
        if ($document->getAccessMode($userobj) >= M_READ) {
            ...
            $app->response()->header("Content-Length", filesize($repo->contentDir . $file->getPath()));
            $app->response()->header("Expires", "0");
            $app->response()->header("Cache-Control", "no-cache, must-revalidate");
            $app->response()->header("Pragma", "no-cache");

            sendFile($repo->contentDir . $file->getPath());
        } else {
            $app->response()->status(403);
            $app->response()->header('Content-Type', 'application/json');
            echo json_encode(array('success'=>false, 'message'=>'No access', 'data'=>''));
...
function getDocumentPreview($id, $version=0, $width=0) { /* {{{ */
    global $app, $repo, $userobj, $settings;
    require_once "MediaREPO/Preview.php";
    $document = $repo->getDocument($id);
    ...
    if($document) {
        if ($document->getAccessMode($userobj) >= M_READ) {
            ...
            if(!empty($width))
                $previewer = new mediarepo_Preview_Previewer($settings->_cacheDir, $width);
            else
                $previewer = new mediarepo_Preview_Previewer($settings->_cacheDir);
...
function removeDocumentCategory($id, $categoryId) { /* {{{ */
    global $app, $repo, $userobj;
    $document = $repo->getDocument($id);
    $category = $repo->getDocumentCategory($categoryId);
    ...
    switch($mode) {
        case 'typeahead';
             ...
             * name will be preceeded by a 'F' or 'D'
                ...
                $tmp = array();
                if(get_class($entry) == 'mediarepo_Core_Document') {
                    $tmp['type'] = 'folder';
                } else {
                    $tmp['type'] = 'document';
                }
...
//                array_unshift($recs, array('type'=>'', 'id'=>0, 'name'=>$querystr, 'comment'=>''));
        ...
        default:
            ...
            foreach ($entries as $entry) {
                ...
                } elseif(get_class($entry) == 'mediarepo_Core_Folder') {
                    $folder = $entry;
...
function createUser() { /* {{{ */
    ...
    $roleid = $role == 'admin' ? mediarepo_Core_User::role_admin : ($role == 'guest' ? mediarepo_Core_User::role_guest : mediarepo_Core_User::role_user);

    $newAccount = $repo->addUser($userName, $password, $fullname, $email, $language, $theme, $comment, $roleid);
    if ($newAccount === false) {
        $app->response()->status(500);
        $app->response()->header('Content-Type', 'application/json');
...
function changeUserPassword($id) { /* {{{ */
    ...
    if(ctype_digit($id))
        $account = $repo->getUser($id);
    else {
        $account = $repo->getUserByLogin($id);
    }
...
function createGroup() { /* {{{ */
    ...
    $newGroup = $repo->addGroup($groupName, $comment);
    if ($newGroup === false) {
        $app->response()->status(500);
        $app->response()->header('Content-Type', 'application/json');
...
function getGroup($id) { /* {{{ */
    ...
    checkIfAdmin();
    if(ctype_digit($id))
        $group = $repo->getGroup($id);
    else {
        $group = $repo->getGroupByName($id);
    }
    if($group) {
...
function changeGroupMembership($id, $operationType) { /* {{{ */
    ...
    if(ctype_digit($userId))
        ...
        $user = $repo->getUserByLogin($userId);
    }
...
function setFolderInheritsAccess($id) { /* {{{ */
    ...
    if(ctype_digit($id))
        ...
        $folder = $repo->getFolderByName($id);
    }
    ...
    if($folder) {
        ...
        $folder = null;
        // reread from db
        $folder = $repo->getFolder($folderId);
...
function changeFolderAccess($id, $operationType, $userOrGroup) { /* {{{ */
    ...
    {
        $userOrGroupObj = $repo->getUserByLogin($userOrGroupIdInput);
    }
    if(!ctype_digit($userOrGroupIdInput) && $userOrGroup == 'group')
    {
        $userOrGroupObj = $repo->getGroupByName($userOrGroupIdInput);
    }
    if(ctype_digit($userOrGroupIdInput) && $userOrGroup == 'user')
    {
        $userOrGroupObj = $repo->getUser($userOrGroupIdInput);
    }
    if(ctype_digit($userOrGroupIdInput) && $userOrGroup == 'group')
    {
        $userOrGroupObj = $repo->getGroup($userOrGroupIdInput);
    }
    if (!$userOrGroupObj) {
        $app->response()->status(404);
        return;
...
function getCategories() { /* {{{ */
    global $app, $repo, $userobj;
		...
		if(false === ($categories = $repo->getDocumentCategories())) {
        $app->response()->status(500);
	  		$app->response()->header('Content-Type', 'application/json');
...
function getCategory($id) { /* {{{ */
    ...
    $category = $repo->getDocumentCategory($id);
    if($category) {
        $data = array();
...
function createCategory() { /* {{{ */
    ...
    $catobj = $repo->getDocumentCategoryByName($category);
    if($catobj) {
        $app->response()->status(409);
        $app->response()->header('Content-Type', 'application/json');
        echo json_encode(array('success'=>false, 'message'=>'Category already exists', 'data'=>''));
    } else {
        if($data = $repo->addDocumentCategory($category)) {
            $app->response()->status(201);
            $app->response()->header('Content-Type', 'application/json');
...
function deleteCategory($id) { /* {{{ */
    ...
    $app->response()->header('Content-Type', 'application/json');
    if($category = $repo->getDocumentCategory($id)) {
...
function getAttributeDefinitions() { /* {{{ */
    ...
    $attrdefs = $repo->getAllAttributeDefinitions();
    $data = [];
...
function changeAttributeDefinitionName($id) { /* {{{ */
    ...
    $attrdef = null;
    if(ctype_digit($id))
        $attrdef = $repo->getAttributeDefinition($id);
