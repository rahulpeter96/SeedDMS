...
//		console.log($(ev.target).parent().children('[name=missing-lang-translation]').val());
	...
	$('div.ajax').each(function(index) { /* {{{ */
		...
		if(view && action) {
			url = mediarepo_webroot+base+"out/out."+view+".php?action="+action;
			if(query) {
				url += "&"+query;
			}
		} else
...
(function( mediarepoUpload, $, undefined ) { /* {{{ */
	...
	mediarepoUpload.setUrl = function(url)  {
		ajaxurl = url;
	}

	mediarepoUpload.setAbortBtnLabel = function(label)  {
		abortBtnLabel = label;
	}

	mediarepoUpload.setEditBtnLabel = function(label)  {
		editBtnLabel = label;
	}

	mediarepoUpload.setMaxFileSize = function(size)  {
		maxFileSize = size;
	}

	mediarepoUpload.setMaxFileSizeMsg = function(msg)  {
		maxFileSizeMsg = msg;
	}
...
//		$('.statusbar').empty();
	...
	mediarepoUpload.handleFileUpload = function(files,obj) {
...
}( window.mediarepoUpload = window.mediarepoUpload || {}, jQuery )); /* }}} */
...
$(document).ready(function() { /* {{{ */
	...
	obj.on('drop', function (e) {
		...
		mediarepoUpload.handleFileUpload(files,obj);
	});
