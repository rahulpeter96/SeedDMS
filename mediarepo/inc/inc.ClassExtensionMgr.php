 ...
 *
 * MediaREPO can be extended by extensions. Extension usually implement
 * hook.
 *
 * @category   REPO
 * @package    MediaREPO
...
class mediarepo_Extension_Mgr {
	 ...
	 * @param boolean $noconstraints set to true if constraints to local mediarepo
	...
	public function checkExtension($dir, $options=array()) { /* {{{ */
		...
		if(!isset($extconf['constraints']['depends']['mediarepo'])) {
			$this->errmsgs[] = "Missing dependency on MediaREPO";
		}
		...
		if(!isset($options['noconstraints']) || $options['noconstraints'] == false) {
			if(isset($extconf['constraints']['depends'])) {
				foreach($extconf['constraints']['depends'] as $dkey=>$dval) {
					switch($dkey) {
					case 'mediarepo':
						$version = new mediarepo_Version;
						$tmp = explode('-', $dval, 2);
						if(self::cmpVersion($tmp[0], $version->version()) > 0 || ($tmp[1] && self::cmpVersion($tmp[1], $version->version()) < 0))
							$this->errmsgs[] = sprintf("Incorrect MediaREPO version (needs version %s)", $extconf['constraints']['depends']['mediarepo']);
						break;
					case 'php':
						$tmp = explode('-', $dval, 2);
	 ...
	 *
	 * Run mediarepo_Extension_Mgr::updateExtensionList() to ensure the
	...
	public function updateExtensionList($version='', $force=false) { /* {{{ */
		if($this->reposurl) {
			if(!file_exists($this->cachedir."/".self::repos_list_file) || $force) {
				$file = @file_get_contents($this->reposurl.($version ? '?mediarepo_version='.$version : ''));
