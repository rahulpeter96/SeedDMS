 ...
 * @version    Release: @package_version@
 */
class mediarepo_LdapAuthentication extends mediarepo_Authentication {
	/**
		...
		if (!is_bool($ds)) {
			...
			if (isset($settings->_ldapType)) {
				if ($settings->_ldapType==1) {
					...
					// See https://sourceforge.net/p/mediarepo/discussion/general/thread/19c70d8d/
				...
				if (is_null($user) && !$settings->_restricted) {
					...
					if (!is_bool($search)) {
						...
						if (!is_bool($info) && $info["count"]==1 && $info[0]["count"]>0) {
							$user = $repo->addUser($username, null, $info[0]['cn'][0], $info[0]['mail'][0], $settings->_language, $settings->_theme, "", 0);
						}
					}
				}
			} elseif($user) {
