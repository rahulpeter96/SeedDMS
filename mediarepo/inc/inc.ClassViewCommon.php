...
class mediarepo_View_Common {
	...
	function callHook($hook) { /* {{{ */
		...
		$ret = null;
		if(isset($GLOBALS['mediarepo_HOOKS']['view'][lcfirst($tmp[2])])) {
			foreach($GLOBALS['mediarepo_HOOKS']['view'][lcfirst($tmp[2])] as $hookObj) {
	...
	function getHookObjects($classname='') { /* {{{ */
		if($classname)
			...
			$tmp = explode('_', get_class($this));
		if(isset($GLOBALS['mediarepo_HOOKS']['view'][lcfirst($tmp[2])])) {
			return $GLOBALS['mediarepo_HOOKS']['view'][lcfirst($tmp[2])];
		}
		return array();
	} /* }}} */
