...
/* deprecated! use mediarepo_Core_File::format_filesize() instead */
 ...
 * older versions
 */
function myrepoDecodeString($string) { /* {{{ */
...
function createVersionigFile($document) { /* {{{ */
	...
	if (!file_exists($repo->contentDir . $document->getDir()))
		if (!mediarepo_Core_File::makeDir($repo->contentDir . $document->getDir())) return false;

	$handle = fopen($repo->contentDir . $document->getDir() .$settings-> _versioningFileName , "wb");
 ...
 * data in the REPO should always use POST because it is harder to run
