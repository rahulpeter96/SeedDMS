 ...
 * @version    Release: @package_version@
 */
class mediarepo_EmailNotify extends mediarepo_Notify {
	/**
	 * Instanz of REPO
	 */
	protected $_repo;
	...
	function __construct($repo, $from_address='', $smtp_server='', $smtp_port='', $smtp_username='', $smtp_password='') { /* {{{ */
		$this->_repo = $repo;
	...
	function toIndividual($sender, $recipient, $subject, $message, $params=array()) { /* {{{ */
		if(is_object($recipient) && !strcasecmp(get_class($recipient), $this->_repo->getClassname('user')) && !$recipient->isDisabled() && $recipient->getEmail()!="") {
		...
		$returnpath = '';
		if(is_object($sender) && !strcasecmp(get_class($sender), $this->_repo->getClassname('user'))) {
	...
	function toGroup($sender, $groupRecipient, $subject, $message, $params=array()) { /* {{{ */
		if ((!is_object($sender) && strcasecmp(get_class($sender), $this->_repo->getClassname('user'))) ||
				(!is_object($groupRecipient) || strcasecmp(get_class($groupRecipient), $this->_repo->getClassname('group')))) {
			return -1;
		}
