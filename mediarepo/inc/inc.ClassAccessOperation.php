 ...
 * @version    Release: @package_version@
 */
class mediarepo_AccessOperation {
	/**
	 * @var object $repo reference to repo
	 * @access protected
	 */
	private $repo;
	...
	function __construct($repo, $obj, $user, $settings) { /* {{{ */
		$this->repo = $repo;
		$this->obj = $obj;
		$this->user = $user;
	...
	function mayEditVersion($vno=0) { /* {{{ */
		if(get_class($this->obj) == $this->repo->getClassname('document')) {
			if($vno)
	...
	function mayUpdateReview($updateUser) { /* {{{ */
		if(get_class($this->obj) == 'mediarepo_Core_Document') {
