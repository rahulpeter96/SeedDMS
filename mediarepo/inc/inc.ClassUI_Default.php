...
class UI_Default {
	...
	function htmlStartPage($title="", $bodyClass="") { /* {{{ */
		...
		if(file_exists("../themes/".$theme."/HTMLHead.html")) {
			...
			echo "<title>".(strlen($settings->_siteName)>0 ? $settings->_siteName : "MediaREPO").(strlen($title)>0 ? ": " : "").htmlspecialchars($title)."</title>\n";
			echo "</head>\n";
	...
	function globalBanner() { /* {{{ */
		...
		echo "   <a class=\"brand\">".(strlen($settings->_siteName)>0 ? $settings->_siteName : "MediaREPO")."</a>\n";
		echo "  </div>\n";
		echo " </div>\n";
		echo "</div>\n";
	...
	function __globalNavigation($folder=null) { /* {{{ */
		...
		if ($folder!=null && is_object($folder) && !strcasecmp(get_class($folder), "mediarepo_Core_Folder")) {
		...
		echo "<div class=\"siteName\">".
			(strlen($settings->_siteName)>0 ? $settings->_siteName : "MediaREPO").
			"</div>\n";
	...
	function __folderNavigationBar($folder) { /* {{{ */
		...
		if (!is_object($folder) || strcasecmp(get_class($folder), "mediarepo_Core_Folder")) {
...
<?php
	...
	function __printFoldersTree($accessMode, $exclude, $folderID, $currentFolderID=-1, $navigation=false) {	/* {{{ */
		global $repo, $user, $form, $settings;
		...
		if ($settings->_expandFolderTree==2){
			...
			if ($currentFolderID!=-1){
				...
				$currentFolder=$repo->getFolder($currentFolderID);
