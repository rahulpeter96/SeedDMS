 ...
 *
 * MediaREPO uses its own simple session management, storing sessions
...
class mediarepo_Session {
	 ...
	 * @return object instance of mediarepo_Session
	 */
...
class mediarepo_SessionMgr {
	 ...
	 *
	 * @param object $db object to access the underlying database
	 * @return object instance of mediarepo_SessionMgr
	 */
	function __construct($db) { /* {{{ */
		$this->db = $db;
	} /* }}} */
	...
	function getAllSessions() { /* {{{ */
		...
		foreach($resArr as $rec) {
			$session = new mediarepo_Session($this->db);
			$session->load($rec['id']);
			$sessions[] = $session;
		}
		return $sessions;
