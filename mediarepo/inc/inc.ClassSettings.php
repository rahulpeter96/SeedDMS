...
class Settings { /* {{{ */
	...
	// Name of site
	var $_siteName = "MediaREPO";
	// Message to display at the bottom of every page.
	var $_footNote = "MediaREPO free document management \"system - www.mediarepo.org";
	...
	// Path to where MediaREPO is located
	var $_rootDir = null;
	// Path to mediarepo_Core
	var $_coreDir = null;
	// Path to mediarepo_Lucene
	var $_luceneClassDir = null;
	// The relative path in the URL, after the domain part.
	var $_httpRoot = "/mediarepo/";
	...
	// database where the tables for myrepo are stored (optional - see adodb-readme)
  ...
  function setXMLAttributValue($node, $attributName, $attributValue) { /* {{{ */
	 ...
	 * If none was found a final try will be made checking /etc/mediarepo
	...
	static function getConfigDir() { /* {{{ */
		...
		if(!$configDir) {
			if(file_exists('/etc/mediarepo'))
				$configDir = '/etc/mediarepo';
		}
		return $configDir;
		/* new code ends here */
	...
	function check($minversion) { /* {{{ */
		...
		if($this->_coreDir) {
			...
			$found = Settings::findInIncPath('MediaREPO/Core.php');
			if(!$found) {
				$result["coreDir"] = array(
					"status" => "notfound",
					"type" => "error",
			...
			try {
				...
				if($dsn) {
					...
					if(!isset($result["coreDir"])) {
						/* Instanciate mediarepo_Core to check version */
						if(!empty($this->_coreDir))
							require_once($this->_coreDir.'/Core.php');
						else
							require_once('MediaREPO/Core.php');
						$tmpcore = new mediarepo_Core_REPO(null, $this->_contentDir);
						$db = new mediarepo_Core_DatabaseAccess($this->_dbDriver, $this->_dbHostname, $this->_dbUser, $this->_dbPass, $this->_dbDatabase);
						if(!$db->connect()) {
						...
						/*
							$repo = new mediarepo_Core_REPO($db, $this->_contentDir.$this->_contentOffsetDir);

							if(!$repo->checkVersion()) {
								$result["dbVersion"] = array(
									"status" => "error",
									"type" => "error",
									"currentvalue" => $repo->version,
