...
$extMgr = new mediarepo_Extension_Mgr($settings->_rootDir."/ext", $settings->_cacheDir, $settings->_repositoryUrl);
...
foreach($EXT_CONF as $extname=>$extconf) {
	if(!isset($extconf['disable']) || $extconf['disable'] == false) {
		...
		if(!empty($extconf['constraints']['depends']['mediarepo'])) {
			$t = explode('-', $extconf['constraints']['depends']['mediarepo'], 2);
			if(mediarepo_Extension_Mgr::cmpVersion($t[0], $version->version()) > 0 || ($t[1] && mediarepo_Extension_Mgr::cmpVersion($t[1], $version->version()) < 0))
