 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_ViewDocument extends mediarepo_Bootstrap_Style {
...
<?php
		...
		case mediarepo_Core_AttributeDefinition::type_url:
	...
	function documentListItem() { /* {{{ */
		...
		if($document) {
			if ($document->getAccessMode($user) >= M_READ) {
				$previewer = new mediarepo_Preview_Previewer($cachedir, $previewwidth);
...
?>
		...
		<td><?php print mediarepo_Core_File::format_filesize($document->getUsedDiskSpace());?></td>
		</tr>
		<tr>
...
<?php
	...
	<?php
		...
		if($converttopdf) {
			$pdfpreviewer = new mediarepo_Preview_PdfPreviewer($cachedir, $timeout);
...
<?php
		...
		$links = mediarepo_Core_REPO::filterDocumentLinks($user, $links, 'target');

		/* Retrieve reverse linked documents */
		$reverselinks = $document->getReverseDocumentLinks();
		$reverselinks = mediarepo_Core_REPO::filterDocumentLinks($user, $reverselinks, 'source');
...
<?php
		...
		$file_exists=file_exists($repo->contentDir . $latestContent->getPath());
		...
		print "</ul>";
		*/
		$previewer = new mediarepo_Preview_Previewer($cachedir, $previewwidthdetail, $timeout);
		...
		if ($file_exists)
			print "<li>". mediarepo_Core_File::format_filesize($latestContent->getFileSize()) .", ".htmlspecialchars($latestContent->getMimeType())."</li>";
		...
		*/
		...
		if($user->isAdmin()) {
			...
			foreach($statuslog as $entry) {
				if($suser = $repo->getUser($entry['userID']))
			...
			foreach ($reviewStatus as $r) {
				...
				switch ($r["type"]) {
					case 0: // Reviewer is an individual.
						$required = $repo->getUser($r["required"]);
				...
				$updateUser = $repo->getUser($r["userID"]);
			...
			foreach ($approvalStatus as $a) {
				...
				switch ($a["type"]) {
					case 0: // Approver is an individual.
						$required = $repo->getUser($a["required"]);
						if (!is_object($required)) {
					...
					case 1: // Approver is a group.
						$required = $repo->getGroup($a["required"]);
						if (!is_object($required)) {
				...
				$updateUser = $repo->getUser($a["userID"]);
...
<?php
			...
			$this->contentContainerStart();
			if($user->isAdmin()) {
				if(mediarepo_Core_REPO::checkIfEqual($workflow->getInitState(), $latestContent->getWorkflowState())) {
...
<?php
			...
			for ($i = count($versions)-2; $i >= 0; $i--) {
				...
				// verify if file exists
				$file_exists=file_exists($repo->contentDir . $version->getPath());
				...
				if ($file_exists) print "<li>". mediarepo_Core_File::format_filesize($version->getFileSize()) .", ".htmlspecialchars($version->getMimeType())."</li>";
				else print "<li><span class=\"warning\">".getMLText("document_deleted")."</span></li>";
...
<?php
		...
		if (count($files) > 0) {
			...
			foreach($files as $file) {
				...
				$file_exists=file_exists($repo->contentDir . $file->getPath());
				...
				if ($file_exists)
					print "<li>".mediarepo_Core_File::format_filesize(filesize($repo->contentDir . $file->getPath())) ." bytes, ".htmlspecialchars($file->getMimeType())."</li>";
