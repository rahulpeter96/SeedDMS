 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_Tasks extends mediarepo_Bootstrap_Style {
	...
	private function __myTasks() { /* {{{ */
		...
		if($workflowmode == 'traditional' || $workflowmode == 'traditional_only_approval') {
			$resArr = $repo->getDocumentList('ApproveByMe', $user);
			if($resArr) {
			...
			$resArr = $repo->getDocumentList('ReviewByMe', $user);
			if($resArr) {
				foreach ($resArr as $res) {
					$document = $repo->getDocument($res["id"]);
					if($document->getAccessMode($user) >= M_READ && $document->getLatestContent()) {
			...
			$resArr = $repo->getDocumentList('WorkflowByMe', $user);
			if($resArr) {
				foreach ($resArr as $res) {
					$document = $repo->getDocument($res["id"]);
					if($document->getAccessMode($user) >= M_READ && $document->getLatestContent()) {
...
//		$content .= "   </ul>\n";
	...
	public function menuClipboard() { /* {{{ */
		...
		foreach($clipboard['folders'] as $folderid) {
			if($folder = $this->params['repo']->getFolder($folderid))
		...
		foreach($clipboard['docs'] as $docid) {
			if($document = $this->params['repo']->getDocument($docid))
	...
	public function mainClipboard() { /* {{{ */
		...
		if($clipboard['docs']) {
			foreach($clipboard['docs'] as $docid) {
				...
				if($document = $repo->getDocument($docid)) {
