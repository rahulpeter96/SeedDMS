 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_UsrMgr extends mediarepo_Bootstrap_Style {
...
<?php
	...
	function info() { /* {{{ */
		...
		if($seluser) {
			$sessionmgr = new mediarepo_SessionMgr($repo->getDB());
			...
			if($quota) {
				...
				echo mediarepo_Core_File::format_filesize($seluser->getUsedDiskSpace())." / ".mediarepo_Core_File::format_filesize($qt)."<br />";
				echo $this->getProgressBar($seluser->getUsedDiskSpace(), $qt);
			} else {
				echo mediarepo_Core_File::format_filesize($seluser->getUsedDiskSpace())."<br />";
			}
			echo "</td></tr>\n";
			...
			if($workflowmode == "traditional") {
				$resArr = $repo->getDocumentList('ReviewByMe', $seluser);
				if($resArr) {
					foreach ($resArr as $res) {
						$document = $repo->getDocument($res["id"]);
			...
			if($workflowmode == "traditional" || $workflowmode == 'traditional_only_approval') {
				$resArr = $repo->getDocumentList('ApproveByMe', $seluser);
				if($resArr) {
					foreach ($resArr as $res) {
						$document = $repo->getDocument($res["id"]);
						if($document->getAccessMode($user) >= M_READ && $document->getLatestContent()) {
...
?>
		...
		<tr>
			...
			<td><select name="role"><option value="<?php echo mediarepo_Core_User::role_user ?>"><?php printMLText("role_user"); ?></option><option value="<?php echo mediarepo_Core_User::role_admin ?>" <?php if($currUser && $currUser->getRole() == mediarepo_Core_User::role_admin) echo "selected"; ?>><?php printMLText("role_admin"); ?></option><option value="<?php echo mediarepo_Core_User::role_guest ?>" <?php if($currUser && $currUser->getRole() == mediarepo_Core_User::role_guest) echo "selected"; ?>><?php printMLText("role_guest"); ?></option></select></td>
		</tr>
		<tr>
...
?>
		...
		<tr>
			...
			<td><?php $this->printFolderChooser("form".($currUser ? $currUser->getId() : '0'), M_READ, -1, $currUser ? $repo->getFolder($currUser->getHomeFolder()) : 0, 'homefolder');?></td>
		</tr>
		<tr>
...
<?php
	if($quota > 0)
		echo $this->warningMsg(getMLText('current_quota', array('quota'=>mediarepo_Core_File::format_filesize($quota))));
	else
