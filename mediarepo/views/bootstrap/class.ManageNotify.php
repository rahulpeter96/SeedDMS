 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_ManageNotify extends mediarepo_Bootstrap_Style {
	...
	function printFolderNotificationList($notifications, $deleteaction=true) { /* {{{ */
		if (count($notifications)==0) {
			...
			foreach($notifications as $notification) {
				$fld = $this->repo->getFolder($notification->getTarget());
				if (is_object($fld)) {
					$owner = $fld->getOwner();
	...
	function printDocumentNotificationList($notifications,$deleteaction=true) { /* {{{ */
		...
		if (count($notifications)==0) {
			...
			$previewer = new mediarepo_Preview_Previewer($this->cachedir, $this->previewwidth, $this->timeout);
			...
			print "</tr></thead>\n<tbody>\n";
			foreach ($notifications as $notification) {
				$doc = $this->repo->getDocument($notification->getTarget());
				if (is_object($doc)) {
					$owner = $doc->getOwner();
