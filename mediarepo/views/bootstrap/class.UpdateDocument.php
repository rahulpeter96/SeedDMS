 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_UpdateDocument extends mediarepo_Bootstrap_Style {
...
<?php
	...
	function show() { /* {{{ */
		...
		if($enablelargefileupload) {
			if($maxuploadsize) {
				$msg = getMLText("max_upload_size").": ".mediarepo_Core_File::format_filesize($maxuploadsize);
			} else {
				$msg = '';
			}
		} else {
...
<?php
	$attrdefs = $repo->getAllAttributeDefinitions(array(mediarepo_Core_AttributeDefinition::objtype_documentcontent, mediarepo_Core_AttributeDefinition::objtype_all));
	if($attrdefs) {
...
<?php
				...
				if($res) {
					...
					foreach ($res as $r) {
						if($r['reviewerUserID'] > 0) {
							$u = $repo->getUser($r['reviewerUserID']);
				...
				foreach($res as $r) {
					if($r['reviewerUserID']) {
						...
						if(!$hasAccess) {
							$noAccessUser = $repo->getUser($r['reviewerUserID']);
...
<?php
				...
				if($res) {
					...
					foreach ($res as $r) {
						if($r['reviewerGroupID'] > 0) {
							$u = $repo->getGroup($r['reviewerGroupID']);
				...
				foreach($res as $r) {
					if ($r['reviewerGroupID']) {
						...
						if(!$hasAccess) {
							$noAccessGroup = $repo->getGroup($r['reviewerGroupID']);
...
<?php
				...
				if($res) {
					...
					foreach ($res as $r) {
						if($r['approverUserID'] > 0) {
							$u = $repo->getUser($r['approverUserID']);
							$tmp[] =  htmlspecialchars($u->getFullName().' ('.$u->getLogin().')');
						}
					}
					if($tmp) {
				...
				foreach($res as $r) {
					if($r['approverUserID']) {
						...
						if(!$hasAccess) {
							$noAccessUser = $repo->getUser($r['approverUserID']);
...
<?php
				...
				if($res) {
					...
					foreach ($res as $r) {
						if($r['approverGroupID'] > 0) {
							$u = $repo->getGroup($r['approverGroupID']);
							$tmp[] =  htmlspecialchars($u->getName());
						}
					}
					if($tmp) {
				...
				foreach($res as $r) {
					if ($r['approverGroupID']) {
						...
						if(!$hasAccess) {
							$noAccessGroup = $repo->getGroup($r['approverGroupID']);
