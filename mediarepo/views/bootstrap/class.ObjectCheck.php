 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_ObjectCheck extends mediarepo_Bootstrap_Style {

	function tree($repo, $folder, $repair, $path=':', $indent='') { /* {{{ */
		global $user;
		...
		foreach($subfolders as $subfolder) {
			$this->tree($repo, $subfolder, $repair, $path.$folder->getId().':', $indent.'  ');
		}
		$path .= $folder->getId().':';
		...
		foreach($documents as $document) {
			...
			if($versions) {
				foreach($versions as $version) {
					$filepath = $repo->contentDir . $version->getPath();
