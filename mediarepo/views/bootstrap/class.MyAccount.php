 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_MyAccount extends mediarepo_Bootstrap_Style {
	...
	function show() { /* {{{ */
		...
		if($quota > 0) {
			if(($remain = checkQuota($user)) < 0) {
				$this->warningMsg(getMLText('quota_warning', array('bytes'=>mediarepo_Core_File::format_filesize(abs($remain)))));
			}
		}
		...
		print "<td>".mediarepo_Core_File::format_filesize($user->getUsedDiskSpace())."</td>\n";
		print "</tr>\n";
		if($quota > 0) {
			print "<tr>\n";
			print "<td>".getMLText("quota")." : </td>\n";
			print "<td>".mediarepo_Core_File::format_filesize($user->getQuota())."</td>\n";
			print "</tr>\n";
