 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_MyDocuments extends mediarepo_Bootstrap_Style {
		...
		if ($showInProcess){
			...
			if($workflowmode == 'traditional' || $workflowmode == 'traditional_only_approval') {
					...
					if($workflowmode == 'traditional') {
						...
						foreach ($reviewStatus["indstatus"] as $st) {
							...
							if ( $st["status"]==0 && isset($docIdx[$st["documentID"]][$st["version"]]) && !in_array($st["documentID"], $dList) ) {
								$dList[] = $st["documentID"];
								$document = $repo->getDocument($st["documentID"]);
				...
				if (count($resArr)>0) {
					...
					foreach ($resArr as $res) {
						$document = $repo->getDocument($res["documentID"]);
						$document->verifyLastestContentExpriry();
				...
				if (strlen($docCSV)>0) {
					...
					foreach ($workflowStatus["u"] as $st) {
						...
						if ( isset($docIdx[$st["document"]][$st["version"]]) && !in_array($st["document"], $dList) ) {
							$dList[] = $st["document"];
							$document = $repo->getDocument($st["document"]);
							$document->verifyLastestContentExpriry();
...
//						print "<td>".getOverallStatusText($res["status"])."</td>";
			...
			/* Documents expired */
			if($docs = $repo->getDocumentsExpired(-3*365, $user)) {
			...
			if (count($resArr)>0) {
				...
				foreach ($docs as $document) {
					...
					print "<td>".$status["date"]." ".htmlspecialchars($repo->getUser($status["userID"])->getFullName())."</td>";
