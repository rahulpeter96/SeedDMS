<?php
//This is the root CSS File for the project!
//Add all the links and files to be included in the html

class mediarepo_Bootstrap_Style extends mediarepo_View_Common {
	var $imgpath;
	...
	function htmlStartPage($title="", $bodyClass="", $base="", $httpheader=array()) { /* {{{ */
		...
		$hookObjs = $this->getHookObjects('mediarepo_View_Bootstrap');
...
<?php
		...
		echo "<title>".(strlen($sitename)>0 ? $sitename : "MediaREPO").(strlen($title)>0 ? ": " : "").htmlspecialchars($title)."</title>\n";
		echo "</head>\n";
	...
	function htmlEndPage($nofooter=false) { /* {{{ */
		...
		if($this->footerjs) {
			...
			if(!is_dir($this->params['cachedir'].'/js')) {
				mediarepo_Core_File::makeDir($this->params['cachedir'].'/js');
			}
			if(is_dir($this->params['cachedir'].'/js')) {
	...
	function webrootjs() { /* {{{ */
		...
		echo "var mediarepo_absbaseprefix=\"".$this->params['absbaseprefix']."\";\n";
		echo "var mediarepo_webroot=\"".$this->params['settings']->_httpRoot."\";\n";
	} /* }}} */
	...
	function missingLanguageKeys() { /* {{{ */
		...
		if($MISSING_LANG) {
			...
			echo "<p><strong>This page contains missing translations in the selected language. Please help to improve MediaREPO and provide the translation.</strong></p>";
			echo "</div>";
	...
	function globalBanner() { /* {{{ */
		...
		echo "  <div class=\"container-fluid\">\n";
		echo "   <a class=\"brand\" href=\"../out/out.ViewFolder.php?folderid=".$this->params['rootfolderid']."\">".(strlen($this->params['sitename'])>0 ? $this->params['sitename'] : "MediaREPO")."</a>\n";
		echo "  </div>\n";
		echo " </div>\n";
		echo "</div>\n";
	} /* }}} */
...
//				echo $this->menuTasks(array('review'=>array(), 'approval'=>array(), 'receipt'=>array(), 'revision'=>array()));
			...
			if($this->params['dropfolderdir'] && $this->params['enabledropfolderlist']) {
				...
				if ($folder!=null && is_object($folder) && !strcasecmp(get_class($folder), $repo->getClassname('folder')))
...
//			}
	...
	private function folderNavigationBar($folder) { /* {{{ */
		$repo = $this->params['repo'];
		if (!is_object($folder) || strcasecmp(get_class($folder), $repo->getClassname('folder'))) {
			echo "<ul class=\"nav\">\n";
			echo "</ul>\n";
			return;
		}
...
<?php
			break;
		case mediarepo_Core_AttributeDefinition::type_email:
			$objvalue = $attribute ? (is_object($attribute) ? $attribute->getValue() : $attribute) : '';
		...
		default:
			if($valueset = $attrdef->getValueSetAsArray()) {
				...
				if(strlen($objvalue) > 80) {
					...
					echo "<input type=\"text\" id=\"".$fieldname."_".$attrdef->getId()."\" name=\"".$fieldname."[".$attrdef->getId()."]\" value=\"".htmlspecialchars($objvalue)."\"".((!$norequire && $attrdef->getMinValues() > 0) ? ' required' : '').($attrdef->getType() == mediarepo_Core_AttributeDefinition::type_int ? ' data-rule-digits="true"' : '')." />";
				}
			}
			break;
		}
	...
	function printNewTreeNavigationJs($folderid=0, $accessmode=M_READ, $showdocs=0, $formid='form1', $expandtree=0, $orderby='') { /* {{{ */
		function jqtree($path, $folder, $user, $accessmode, $showdocs=1, $expandtree=0, $orderby='') {
			if($path || $expandtree) {
				...
				$subfolders = mediarepo_Core_REPO::filterAccess($subfolders, $user, $accessmode);
				$children = array();
				foreach($subfolders as $subfolder) {
					...
					if($expandtree || $pathfolder->getID() == $subfolder->getID()) {
						if($showdocs) {
							...
							$documents = mediarepo_Core_REPO::filterAccess($documents, $user, $accessmode);
		...
		if($folderid) {
			$folder = $this->params['repo']->getFolder($folderid);
			$path = $folder->getPath();
			...
			if(!$folder->hasSubFolders()) {
				...
				if($showdocs) {
					$documents = $folder->getDocuments($orderby);
					$documents = mediarepo_Core_REPO::filterAccess($documents, $this->params['user'], $accessmode);
					foreach($documents as $document) {
						$node2 = array('label'=>$document->getName(), 'id'=>$document->getID(), 'load_on_demand'=>false, 'is_folder'=>false);
						$node['children'][] = $node2;
					}
			...
			$root = $this->params['repo']->getFolder($this->params['rootfolderid']);
...
<?php
	 ...
	 * for deleting a document. In mediarepo version < 4.3.9 this was just a
...
//			$('.delete-document-btn').click(function(ev) {
	 ...
	 * {@link mediarepo_Bootstrap_Style::printDeleteDocumentButton()}
	 *
...
//			$('.delete-attribute-value-btn').click(function(ev) {
	...
	function documentListRow($document, $previewer, $skipcont=false, $version=0) { /* {{{ */
		...
		if($latestContent) {
			...
			$files = mediarepo_Core_REPO::filterDocumentFiles($user, $files);

			/* Retrieve linked documents */
			$links = $document->getDocumentLinks();
			$links = mediarepo_Core_REPO::filterDocumentLinks($user, $links);

			/* Retrieve reverse linked documents */
			$revlinks = $document->getReverseDocumentLinks();
			$revlinks = mediarepo_Core_REPO::filterDocumentLinks($user, $revlinks);

			$content .= "<td>";
			if (file_exists($repo->contentDir . $latestContent->getPath())) {
...
//		$content .= "<td>".htmlspecialchars($owner->getFullName())."</td>";
		...
		if($enableRecursiveCount) {
			...
			$subsub = mediarepo_Core_REPO::filterAccess($subsub, $user, M_READ);
			$subdoc = $subFolder->getDocuments();
			$subdoc = mediarepo_Core_REPO::filterAccess($subdoc, $user, M_READ);
...
<?php
		...
		foreach($statusList as $rec) {
			...
			switch ($rec["type"]) {
				case 0: // individual.
					$required = $repo->getUser($rec["required"]);
				...
				case 1: // Approver is a group.
					$required = $repo->getGroup($rec["required"]);
					if (!is_object($required)) {
			...
			$updateuser = $repo->getUser($rec["userID"]);
			if(!is_object($required))
