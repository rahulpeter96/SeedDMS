 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_WorkflowSummary extends mediarepo_Bootstrap_Style {
	...
	function show() { /* {{{ */
		...
		foreach ($workflowStatus["u"] as $st) {
			...
			$workflow = $repo->getWorkflow($st['workflow']);
			$state = $repo->getWorkflowState($st['state']);
			$owner = $document->getOwner();
			$moduser = $repo->getUser($st['userid']);
		...
		foreach ($workflowStatus["g"] as $st) {
			...
			$state = $repo->getWorkflowState($st['state']);
			$owner = $document->getOwner();
			$modgroup = $repo->getGroup($st['groupid']);
