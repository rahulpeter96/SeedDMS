 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_Search extends mediarepo_Bootstrap_Style {
...
<?php
		if($attrdefs) {
			foreach($attrdefs as $attrdef) {
				$attricon = '';
				if($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_all) {
?>
<tr>
...
// }}}
		...
		if($attrdefs) {
			foreach($attrdefs as $attrdef) {
				$attricon = '';
				if($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_document || $attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_documentcontent) {
...
<?php
		...
		if($attrdefs) {
			foreach($attrdefs as $attrdef) {
				$attricon = '';
				if($attrdef->getObjType() == mediarepo_Core_AttributeDefinition::objtype_folder) {
					if(!empty($attributes[$attrdef->getID()]))
						$openfilterdlg = true;
				}
			}
		...
		if($entries) {
			...
			foreach ($entries as $entry) {
				if(get_class($entry) == $repo->getClassname('document')) {
					$doccount++;
				} elseif(get_class($entry) == $repo->getClassname('document')) {
					$foldercount++;
				}
			}
			 */
