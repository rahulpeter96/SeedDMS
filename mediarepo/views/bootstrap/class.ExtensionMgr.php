 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_ExtensionMgr extends mediarepo_Bootstrap_Style {
...
<?php
	...
	function info() { /* {{{ */
		...
		foreach($list as $re) {
			...
			$needsupdate = !isset($GLOBALS['EXT_CONF'][$re['name']]) || mediarepo_Extension_Mgr::cmpVersion($re['version'], $GLOBALS['EXT_CONF'][$re['name']]['version']) > 0;
			echo "<tr";
