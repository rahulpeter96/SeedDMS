 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_BackupTools extends mediarepo_Bootstrap_Style {
	...
	function show() { /* {{{ */
		...
		if($this->params['hasbackupdir']) {
			...
			print getMLText("space_used_on_data_folder")." : ".mediarepo_Core_File::format_filesize(dskspace($backupdir));
		...
		if($entries) {
			...
			foreach ($entries as $entry){
				...
				$folder=$repo->getFolder((int)$folderid);
				...
				print "<td>".mediarepo_Core_File::format_filesize(filesize($backupdir.$entry))."</td>\n";
				print "<td>";
