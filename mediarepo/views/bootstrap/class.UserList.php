 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_UserList extends mediarepo_Bootstrap_Style {
...
<?php
		foreach ($allUsers as $currUser) {
			...
			case mediarepo_Core_User::role_user:
				printMLText("role_user");
				break;
			case mediarepo_Core_User::role_admin:
				printMLText("role_admin");
				break;
			case mediarepo_Core_User::role_guest:
				printMLText("role_guest");
				break;
			}
			echo "</td>";
			echo "<td>";
			echo mediarepo_Core_File::format_filesize($currUser->getUsedDiskSpace());
			if($quota) {
				echo " / ";
				$qt = $currUser->getQuota() ? $currUser->getQuota() : $quota;
				echo mediarepo_Core_File::format_filesize($qt)."<br />";
