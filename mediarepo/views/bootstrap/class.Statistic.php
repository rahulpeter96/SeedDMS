 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_Statistic extends mediarepo_Bootstrap_Style {
		var $repo;
		var $folder_count;
		var $document_count;
		var $file_count;
	...
	function printFolder($folder) { /* {{{ */
		...
		print "<small>".mediarepo_Core_File::format_filesize($folder_size).", ".$doc_count." ".getMLText("documents")."</small>\n";
	...
	function printDocument($document) { /* {{{ */
		...
		if (file_exists($this->repo->contentDir.$document->getDir())) {
			$handle = opendir($this->repo->contentDir.$document->getDir());
			while ($entry = readdir($handle) ) {
				if (is_dir($this->repo->contentDir.$document->getDir().$entry)) continue;
				else{
					$local_file_count++;
					$folder_size += filesize($this->repo->contentDir.$document->getDir().$entry);
				}
		...
		print "<small>".mediarepo_Core_File::format_filesize($folder_size).", ".$local_file_count." ".getMLText("files")."</small>\n";
...
<?php
		...
		print "<li>".getMLText("storage_size").": ".mediarepo_Core_File::format_filesize($this->storage_size)."</li>\n";
