 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_Indexer extends mediarepo_Bootstrap_Style {
...
<?php
	...
	protected function tree($repo, $index, $indexconf, $folder, $indent='') { /* {{{ */
...
//		echo $indent."D ".htmlspecialchars($folder->getName())."\n";
		...
		foreach($subfolders as $subfolder) {
			$this->tree($repo, $index, $indexconf, $subfolder, $indent.'  ');
		}
...
//			echo $indent."  ".$document->getId().":".htmlspecialchars($document->getName());
			...
			if(!($hit = $lucenesearch->getDocument($document->getId()))) {
				...
				/*
				try {
					$index->addDocument(new $indexconf['IndexedDocument']($repo, $document, $this->converters ? $this->converters : null, false, $this->timeout));
					echo "(document added)";
				} catch(Exception $e) {
				 ...
				 * where added when a new document was added to the repo. In such
...
<?php
		$this->tree($repo, $index, $indexconf, $folder);
		echo "</div>";
