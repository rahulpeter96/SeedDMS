 ...
 * @version    Release: @package_version@
 */
class mediarepo_View_EditOnline extends mediarepo_Bootstrap_Style {
	var $repo;
	var $folder_count;
	var $document_count;
	var $file_count;
...
<?php
		...
		echo $Parsedown->text(file_get_contents($repo->contentDir . $version->getPath()));
?>
			</div>
		  <div class="tab-pane" id="preview_plain">
<?php
		echo "<pre>".htmlspecialchars(file_get_contents($repo->contentDir . $version->getPath()))."</pre>";
?>
			</div>
		</div>
<?php
...
<?php
		echo htmlspecialchars(file_get_contents($repo->contentDir . $version->getPath()));
?>
</textarea>
