*********************************************
How to set up a Synology NAS to run MediaREPO
*********************************************

**This guide has been updated and tested to work on Synology DSM 6.0. It should as well work with older REPO versions, however some steps or paths may be different.**

Introduction
############
MediaREPO is a feature rich and lightweight document management system. Unfortunately, some of the tools which are part of many Linux distros, have not been made available by
Synology and therefore require additional steps to bring them to your Synology.

This guide covers the installation of the required tools to have all features of MediaREPO available. It does not cover the installation of 3rd party programs (like OPKG). It
does not cover the installation of MediaREPO as well, please refer to the separate README.Install.md file.
...
If the tests above are successful, you are ready to use MediaREPO Preview.
...
The following packages are required by MediaREPO:
...
MediaREPO uses other small tools (for example the Slim Framework) to add some additional functionality. At the moment (Version 5.0.x) the list contains the following
tools:
...
To create the fulltext index, MediaREPO needs to be able to convert the documents to text files to read the terms
...
In MediaREPO you can now configure the setting for Word documents to the path where you extracted the files in the step before. Point
...
The symlink must point to docx2txt.sh to get it working. In MediaREPO you can now just configure docx2txt followed by any additional commands.
...
Once completed, xlsx2csv is available and can be configured within MediaREPO.

Complete the installation
*************************

Now you are ready to install MediaREPO and configure the database. Follow the README file to install MediaREPO.