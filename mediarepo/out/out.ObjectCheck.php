...
$unlinkedversions = $repo->getUnlinkedDocumentContent();
$unlinkedfolders = $repo->checkFolders();
$unlinkeddocuments = $repo->checkDocuments();
$nofilesizeversions = $repo->getNoFileSizeDocumentContent();
$nochecksumversions = $repo->getNoChecksumDocumentContent();
$duplicateversions = $repo->getDuplicateDocumentContent();
foreach(array('review', 'approval') as $process) {
	foreach(array('user', 'group') as $ug) {
		$processwithoutusergroup[$process][$ug] = $repo->getProcessWithoutUserGroup($process, $ug);
	}
}
$rootfolder = $repo->getFolder($settings->_rootFolderID);
