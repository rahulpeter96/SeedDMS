...
// Search in Fulltext {{{
	...
	if(isset($_GET['categoryids']) && $_GET['categoryids']) {
		foreach($_GET['categoryids'] as $catid) {
			if($catid > 0) {
				$category = $repo->getDocumentCategory($catid);
				$categories[] = $category;
	...
	if (isset($_GET["ownerid"]) && is_numeric($_GET["ownerid"]) && $_GET["ownerid"]!=-1) {
		$owner = $repo->getUser($_GET["ownerid"]);
		if (!is_object($owner)) {
	...
	if(strlen($query) < 4 && strpos($query, '*')) {
		...
		if($hits === false) {
			...
			if($hits) {
				foreach($hits as $hit) {
					if($tmp = $repo->getDocument($hit['document_id'])) {
	...
	if (isset($_GET["targetid"]) && is_numeric($_GET["targetid"]) && $_GET["targetid"]>0) {
		...
		$startFolder = $repo->getFolder($targetid);
	}
	else {
...
//		$resultmode = 0x01;
	...
	if(isset($_GET['categoryids']) && $_GET['categoryids']) {
		foreach($_GET['categoryids'] as $catid) {
			if($catid > 0)
				$categories[] = $repo->getDocumentCategory($catid);
		}
	}
...
//		$resultmode = 0x01;
	...
	// ---------------- Start searching -----------------------------------------
	$startTime = getTime();
	$resArr = $repo->search($query, 0, 0 /*$limit, ($pageNumber-1)*$limit*/, $mode, $searchin, $startFolder, $owner, $status, $creationdate ? $startdate : array(), $creationdate ? $stopdate : array(), array(), array(), $categories, $attributes, $resultmode, $expirationdate ? $expstartdate : array(), $expirationdate ? $expstopdate : array());
	$searchTime = getTime() - $startTime;
	$searchTime = round($searchTime, 2);
...
if($settings->_showSingleSearchHit && count($entries) == 1) {
	$entry = $entries[0];
	if(get_class($entry) == $repo->getClassname('document')) {
		header('Location: ../out/out.ViewDocument.php?documentid='.$entry->getID());
		exit;
	} elseif(get_class($entry) == $repo->getClassname('folder')) {
		header('Location: ../out/out.ViewFolder.php?folderid='.$entry->getID());
		exit;
	}
} else {
	$tmp = explode('.', basename($_SERVER['SCRIPT_FILENAME']));
	$view = UI::factory($theme, $tmp[1], array('repo'=>$repo, 'user'=>$user, 'query'=>$query, 'searchhits'=>$entries, 'totalpages'=>$totalPages, 'pagenumber'=>$pageNumber, 'searchtime'=>$searchTime, 'urlparams'=>$_GET, 'cachedir'=>$settings->_cacheDir));
	if($view) {
		...
		$attrdefs = $repo->getAllAttributeDefinitions(array(mediarepo_Core_AttributeDefinition::objtype_document, mediarepo_Core_AttributeDefinition::objtype_documentcontent, mediarepo_Core_AttributeDefinition::objtype_folder, mediarepo_Core_AttributeDefinition::objtype_all));
		$view->setParam('attrdefs', $attrdefs);
		$allCats = $repo->getDocumentCategories();
		$view->setParam('allcategories', $allCats);
		$allUsers = $repo->getAllUsers($settings->_sortUsersInList);
