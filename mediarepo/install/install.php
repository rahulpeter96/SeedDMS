...
if (file_exists("../inc/inc.Settings.old.php")) {
  echo "You can't install MediaREPO, unless you delete " . realpath("../inc/inc.Settings.old.php") . ".";
  exit;
}
...
if (!file_exists("create_tables-innodb.sql")) {
  echo "Can't install MediaREPO, 'create_tables-innodb.sql' missing";
  exit;
}
if (!file_exists("create_tables-sqlite3.sql")) {
  echo "Can't install MediaREPO, 'create_tables-sqlite3.sql' missing";
  exit;
}
if (!file_exists("create_tables-postgres.sql")) {
  echo "Can't install MediaREPO, 'create_tables-postgres.sql' missing";
  exit;
}
if (!file_exists("settings.xml.template_install")) {
  echo "Can't install MediaREPO, 'settings.xml.template_install' missing";
  exit;
}
 ...
 */
define("mediarepo_INSTALL", "on");
define("mediarepo_VERSION", "5.1.7");
...
if (!file_exists($configDir."/ENABLE_INSTALL_TOOL")) {
	echo "For installation of MediaREPO, you must create the file conf/ENABLE_INSTALL_TOOL";
	exit;
}
...
UI::globalBanner();
UI::contentStart();
UI::contentHeading("MediaREPO Installation for version ".mediarepo_VERSION);
if(isset($msg))
...
if ($action=="setSettings") {
	 ...
	 */
	$hasError = printCheckError( $settings->check(substr(str_replace('.', '', mediarepo_VERSION), 0,2)));
