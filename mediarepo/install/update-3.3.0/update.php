	...
	foreach($arr as $tblname => $schema) {
		...
		foreach($recs as $rec) {
			foreach($schema['fields'] as $field) {
				if($rec[$field] !== myrepoDecodeString($rec[$field])) {
					$updateSql = "UPDATE ".$tblname." SET `".$field."`=".$db->quote(myrepoDecodeString($rec[$field]))." where ";
					if(isset($schema['key']))
					...
					echo "<tr><td>".$tblname."</td><td>".$field."</td><td>".htmlspecialchars($rec[$field])."</td><td>".htmlspecialchars(myrepoDecodeString($rec[$field]))."</td><td><pre>".htmlspecialchars($updateSql)."</pre></td></tr>\n";
					if($doupdate) {
