...
/**
 * MediaREPO access using WebDAV
 *
 * @access  public
 * @author  Uwe Steinmann <steinm@php.net>
 * @version @package-version@
 */
class HTTP_WebDAV_Server_mediarepo extends HTTP_WebDAV_Server
{
	/**
	 * A reference of the REPO itself
	 *
	 ...
	 *
	 * @access public
	 * @param  object $repo reference to REPO
	 */
	function ServeRequest($repo = null, $logger = null) /* {{{ */
	{
	...
	function check_auth($type, $user, $pass) /* {{{ */
	{
		...
		if (!$userobj && isset($settings->_ldapHost) && strlen($settings->_ldapHost)>0) {
			...
			$authobj = new mediarepo_LdapAuthentication($this->repo, $settings);
		...
		if(!$userobj) {
			...
			$authobj = new mediarepo_DbAuthentication($this->repo, $settings);
			$userobj = $authobj->authenticate($user, $pass);
		} /* }}} */
	...
	function reverseLookup($path) /* {{{ */
	{
		...
		if(!$patharr) {
			if(!$docname) {
				...
				if($this->useorgfilename)
					$document = $this->repo->getDocumentByOriginalFilename($docname, $root);
				else
					$document = $this->repo->getDocumentByName($docname, $root);
				if($document) {
					if($this->logger)
		...
		foreach($patharr as $pathseg) {
			if($folder = $this->repo->getFolderByName($pathseg, $parentfolder)) {
				$parentfolder = $folder;
			}
		}
		if($folder) {
			if($docname) {
				if($this->useorgfilename)
					$document = $this->repo->getDocumentByOriginalFilename($docname, $folder);
				else
					$document = $this->repo->getDocumentByName($docname, $folder);
				if($document) {
					if($this->logger)
						$this->logger->log('reverseLookup: found document '.$document->getName().' ('.$document->getID().')', PEAR_LOG_DEBUG);
					return $document;
	...
	function PROPFIND(&$options, &$files) /* {{{ */
	{
		...
		if (get_class($obj) == $this->repo->getClassname('folder') && !empty($options["depth"])) {

			$subfolders = $obj->getSubFolders();
			$subfolders = mediarepo_Core_REPO::filterAccess($subfolders, $this->user, M_READ);
			if ($subfolders) {
			...
			$docs = mediarepo_Core_REPO::filterAccess($documents, $this->user, M_READ);
	...
	function fileinfo($obj) /* {{{ */
	{
		...
		if (get_class($obj) == $this->repo->getClassname('folder')) {
			// modification time
...
//			$info["path"] = htmlspecialchars($path.rawurlencode($obj->getName()));
			...
			$info["props"][] = $this->mkprop("getcontentlength", filesize($this->repo->contentDir.'/'.$fspath));
			if($keywords = $obj->getKeywords())
				$info["props"][] = $this->mkprop("MediaREPO:", "keywords", $keywords);
			$info["props"][] = $this->mkprop("MediaREPO:", "id", $obj->getID());
			$info["props"][] = $this->mkprop("MediaREPO:", "version", $content->getVersion());
			$status = $content->getStatus();
			$info["props"][] = $this->mkprop("MediaREPO:", "status", $status['status']);
			$info["props"][] = $this->mkprop("MediaREPO:", "status-comment", $status['comment']);
			$info["props"][] = $this->mkprop("MediaREPO:", "status-date", $status['date']);
			if($obj->getExpires())
				$info["props"][] = $this->mkprop("MediaREPO:", "expires", date('c', $obj->getExpires()));
		}
		if($comment = $obj->getComment())
			$info["props"][] = $this->mkprop("MediaREPO:", "comment", $comment);
		$info["props"][] = $this->mkprop("MediaREPO:", "owner", $obj->getOwner()->getLogin());
		...
		if($attributes) {
			foreach($attributes as $attribute) {
				...
				if($valueset && $attrdef->getMultipleValues()) {
					...
					$info["props"][] = $this->mkprop("MediaREPO:", str_replace(' ', '', $attrdef->getName()), $delimiter.implode($delimiter, $attribute->getValueAsArray()));
				} else
					$info["props"][] = $this->mkprop("MediaREPO:", str_replace(' ', '', $attrdef->getName()), $attribute->getValue());
			}
		}
	...
	function GET(&$options) /* {{{ */
	{
		...
		$fspath = $this->repo->contentDir.'/'.$content->getPath();
...
//			$name	 = htmlspecialchars($filename);
	...
	function PUT(&$options) /* {{{ */
	{
		...
		if (!$folder || get_class($folder) != $this->repo->getClassname('folder')) {
			return "409 Conflict";
		}
		...
		if($this->useorgfilename)
			$document = $this->repo->getDocumentByOriginalFilename($name, $folder);
		else
			$document = $this->repo->getDocumentByName($name, $folder);
		if($document) {
			if($this->logger)
			...
			if ($document->getAccessMode($this->user) < M_READWRITE) {
				 ...
				 */
				$lc = $document->getLatestContent();
				if($lc->getChecksum() == mediarepo_Core_File::checksum($tmpFile)) {
					if($this->logger)
			...
			$controller->setParam('repo', $this->repo);
...
//			if(!$res = $folder->addDocument($name, '', 0, $this->user, '', array(), $tmpFile, $name, $fileType, $mimetype, 0, array(), array(), 0, "")) {
	...
	{		   
		...
		if (get_class($folder) != $this->repo->getClassname('folder')) {
			if($this->logger)
		...
		if ($this->repo->getFolderByName($name, $folder) ) {
...
//			if(!$obj->remove()) {
	...
	function MOVE($options) /* {{{ */
	{
		...
		if(get_class($objdest) == $this->repo->getClassname('document')) {
			/* If destination object is a document it must be overwritten */
			if (!$options["overwrite"]) {
				return "412 precondition failed";
			}
			if(get_class($objsource) == $this->repo->getClassname('folder')) {
			...
			return "204 No Content";
		} elseif(get_class($objdest) == $this->repo->getClassname('folder')) {
			/* Set the new Folder of the source object */
			if(get_class($objsource) == $this->repo->getClassname('document'))
				$objsource->setFolder($objdest);
			elseif(get_class($objsource) == $this->repo->getClassname('folder'))
	...
	function COPY($options, $del=false) /* {{{ */
	{
		...
		if (get_class($objsource) == $this->repo->getClassname('folder') && ($options["depth"] != "infinity")) {
				...
				if($prop["ns"] == "MediaREPO:") {
					if(in_array($prop['name'], array('id', 'version', 'status', 'status-comment', 'status-date'))) {
						...
						case "comment":
							...
							break;
						default:
							if($attrdef = $this->repo->getAttributeDefinitionByName($prop["name"])) {
								$valueset = $attrdef->getValueSetAsArray();
